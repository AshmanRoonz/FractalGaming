...
- **(v47.03) `window.__demReport()` — the EARTH DEM STAGE SPLIT.** `_fetchDemRing` profiles as one 17.9 s blob because the JS self-profiler attributes a FUNCTION, not a statement, so it now carries stage timers (`stats.demMs`: net / decode / fill / hist / wet, and wet splits into close / blur) summed across every `LSSEarthTiles` a launch builds - a near patch AND a far patch, each its own instance. **It immediately killed a plan:** the per-texel height decode everyone assumes is the cost is **7%** (269 ms); the blur was **45%** (1,734 ms) and the float-keyed `Map` histogram **18%** (683 ms). v47.04 took those to 826 / 301 ms. ⚠ Every DEM loop is sliced behind `_leSlicer`, so saving CPU there buys main-thread headroom, NOT curtain time, on a warm-network launch. See [`performance_review.md`](performance_review.md).
- **(v47.05-47.07) TWO IN-PLAY GEOMETRY GATES, AND THE PANE-SIZE TRAP THEY PROVE.** `_swTreeVisTick` hides foliage InstancedMeshes past `uTreeFadeB` (the shader already collapses them to degenerate triangles - zero pixels, FULL vertex cost - and the shadow pass has no fade at all, so it submits them at full size); `_clipSkyOccluder` hides clipmap rings that lie entirely outside the opaque camera-centred sky dome (`renderOrder` 3 vs the rings' -2, so the dome paints over them). Together at 1920x1080: **-2.8 M triangles and -47 draw calls a frame, roughly +14 fps.** ⚠⚠ **THE SAME TREE PATCH MEASURES +0.0 fps AT PANE SIZE AND +4.6% AT 1080p.** Interleave the A/B (`window.__treeVisCull`, `window.__clipCull`) on a SETTLED world - the world streams for minutes after spawn and a sequential before/after is worthless - and make sure any debug toggle RESTORES rather than merely stopping (a non-symmetric off-switch measured the same scene twice). ⚠ `_clipWireParents` points each level's atlas at the NEXT level's texture, so a hidden level's atlas is still sampled: hide `L.mesh` only, NEVER skip `_clipBakeLevel`/`_clipBakeEdge`. See [`performance_review.md`](performance_review.md).
- **(v47.08) `window.__clipReport()` — AND THE HITCH THAT WAS NOT ONE.** Hit/miss counters in `_clipCellH` plus per-level bake timers. A bake texel is an array read when it hits the baked height table and a full `_stGroundYCarved` when it misses (FIVE of them on levels >= `_CLIP_PEAK.minLevel`). Flying the hub at 1400 u/s looked damning - 118 snaps, 2,054 ms, worst 28-40 ms each, `hub:clip` the top frame-spike contributor - **but the misses all start past |x| ~ 45,000, and `_HB_BOX = 53248` is commented "45,056 (cities) + 8,192 (L0 half window)": the flight had gone 31% PAST the world the table is sized for.** Re-flown as a 40,000-radius orbit inside the city ring: **100% hits over 4.43 M samples, 0 misses, 177 ms of bake for MORE bakes (3,184 vs 1,855), L3-L5 avg 0.1 ms instead of 6.7-9.1.** No change made; `_HB_BOX`'s sizing is now measured rather than asserted. ⚠ Open: nothing here checks whether a player can actually fly past +/-45,056 - if they can, the cheap fix is a far table at `sp = 32` (L0 is the ONLY clipmap spacing without one), not a bigger base box.
- **(v47.10) RACE GATES: TWO PATHS INSTALL A CIRCUIT, AND ONLY ONE OF THEM HANDED OVER THE OLD MESHES.** `_raceCircuitAdopt` (the net path) ends with `_raceCircuitInstall(); _raceCircuitOnArrive();` - and `_raceCircuitOnArrive` is what destroys the live `RaceRing`s so gameLoop rebuilds them against the new list. `_raceCircuitTrackBuild` (called from `buildRoomGraphLevel`, i.e. on EVERY round rebuild) assigned `game.raceCircuit = C` and called `_raceCircuitInstall()` **without** `_raceCircuitOnArrive()`. Track circuits re-roll per round with a DIFFERENT gate count (measured on `race_shifting`: round 1 = 12 gates, round 2 = 14), and gameLoop only respawns the meshes `if (!game.poleRings.length)` - so round 2 drew round 1's 12 rings at round 1's positions while the HUD, the minimap and the BOTS all read `C.rings`. Owner: *"i couldn't see the first check point, i saw it on the mini map, and the bots got it and moved on"*. **The tell is `__race.info()`: `rings` != `poleRings`.** Fixed at the source, plus a belt in gameLoop that respawns on a count mismatch and logs `[race] gate desync`. ⚠ Any future path that swaps `game.raceCircuit` must call `_raceCircuitOnArrive()`.
- **(v47.11-47.13) THE EARTH WORLD IS KEPT ACROSS A ROUND — `_lssGmapsLevelSig` + `_keepTiles`.** `_lssGmapsBuildLevel` runs on EVERY level build, and a race circuit re-rolls its gates every round, so round 2 used to tear the whole city down (`LSSEarthTiles.dispose()` — **1,692 material disposes**, caught by hooking `Material.prototype.dispose`) and rebuild it from the DEM even though the LOCATION had not moved. JS self-profile over the transition: `boxAny` (the R=8 water-mask closing) **1,740 ms of SELF time**, scene traverse 1,405, getImageData 602, `_fetchDemRing` 519 self / 2,442 total, `_loadBuildingsBody` 1,539, `updateNight` 1,505 — ~5 s of main-thread JS landing as **33 frames over 60 ms, worst 401 ms**. Owner: *"couple hitches in race/earth circuit"*. Same shape as v38.99's `_keepTerrain`: if the signature (streamed-vs-single class, lat/lng, extent, scale, cyberpunk, ads) is unchanged, reuse the world; gates, spawn points and the rest of the build still re-roll. **Measured after: 33 spikes -> 1, worst 401 -> 101 ms, median 8.9, p99 17.1.** `window.__earthKeep = false` for an A/B. ⚠ Keep `_lssGmapsLevelSig` in step with the constructor arguments — a field the constructor reads and the signature does not is a stale world. A typed DROP changes `level.lat/lng` BEFORE the build call, so it correctly rebuilds. **v47.11 also gave that teardown `_lssRetainMat`** (it was the one rebuild the v38.97 program-retention fix never covered: `freed x19`, programs 187->176, then `lambert [RELINK]` and friends). That was real but NOT the main cost — worth knowing, because fixing it alone moved the hitches by nothing.
- **(v47.14) THE EARTH GROUND LIFT — dim the sky without crushing the terrain (`_leApplyGroundLift`).** Owner: *"the ground in the cities [is] very dark, on Earth... the hill is just all black ... we dim the skies in cities to make the cyberpunk buildings look cooler, but maybe there could be a way it doesn't affect the ground so much"*. **MEASURED in Hong Kong:** the earth night rig (`_lssEarthApplyDayNight` -> `_HUB_ZONES.DUSK`) leaves the terrain lit by `AmbientLight 3.73 @ #2b3b90` - a dim BLUE - with key 0.327 / fill 0.151 / rim 0.112 / hemi 0.4, under ACES at exposure 0.78. The TOWERS survive it because their window texels clear the dusk bloom threshold (0.14, strength 1.70) and bloom; bare hillside has no bright texels, so it renders near-black. ⚠ **The facades are `MeshLambertMaterial` with `emissive #000000` and NO emissiveMap, and the terrain is the same class WITH a `map` - so `emissiveMap` is the one channel the ground has and the buildings do not.** The terrain now carries a self-lit floor driven by its own satellite imagery (`emissive` grey, `emissiveMap = map`): it keeps its colour and detail and stops falling to zero, and because it is a floor rather than a fill light, shading and shadows still read on top. **0.30 chosen by eye against 0 / 0.16 / 0.30 / 0.55** at a straight-down vantage over the hill flank - 0.16 still too dark to read streets, 0.55 reads like daylight and fights the night. Live tuner `window.__earthGroundLiftApply(v)`; `window.__earthGroundLift = 0` restores the old look exactly. ⚠ `emissiveMap` is a program-cache-key term (`emissiveMapUv`), so this forks ONE extra terrain program, built behind the curtain. ⚠ The DUSK `tint`/`tintK: 0.90` pair is the HUB's sandwich terrain only (`_hzGrass/_hzRock/_hzSnow/_hzMoss`) - it never touched earth, so do not reach for it here.
- **(v47.15) ⚠⚠ KEEPING A WORLD ACROSS A ROUND MEANS RE-APPLYING EVERY PIECE OF *SHARED SCENE STATE* THE ROUND TEARDOWN RESETS.** The v47.13 `_keepTiles` win came with a regression I shipped and the owner found twice: *"in your preview window, i can see a weird wall, in earth/race"* and *"the water didn't load that time"*. **The mechanism is one sentence: `_swApplyAtmosphere` runs on EVERY round rebuild, its no-sandwich-terrain branch is what a real-world level always takes, and that branch RESETS shared scene state - while the only code that re-applied it lived inside `_lssGmapsBuildLevel`'s "the world was built fresh" branch.** So round 1 was always right and round 2 onward was wrong, which is exactly the shape that makes these look like two unrelated bugs. **Census that branch before adding anything to the keep path** - it resets fog, background, `_clearBrokenSimForcefield`, `_swDisposeHubWater` (*which also `_wxDispose()`s the weather*), `_setArenaGridVisible(true)`, `_skyDomeRefresh`, `_lssApplyHubLighting(false)`, `_lssRestoreArenaMood()`. Two of those were owned by the gmaps build and therefore broken; the sky was already self-healing in `_lssGmapsTick` (*"THE SKY CANNOT BE ALLOWED TO STAY GONE"*, v46.76), and lighting/mood/fog are not set by the gmaps build at all, so the set is complete at two.
  - **THE WALL was the ARENA BOUNDARY GRID** - six `LineSegments` (`userData.isArenaGrid`, 20,402 verts each) forming a **50,000-unit wireframe box at the world origin**, inside a level 297,000 units across, so its faces cut straight through Hong Kong. Fixed by **vetoing inside `_setArenaGridVisible`** rather than at the call sites: showing the grid is a *request*, and a streamed Earth has no arena edge to mark, whoever asks. That is ordering-proof, which chasing callers is not. ⚠ An **overlay** level (`_lssGmaps.overlayOnly`) keeps its grid - it still has a real arena underneath. ⚠ `_lssGmaps.cleanup` restores the grid, and it ran while `active` was still `true` (every caller clears the flag *after* calling it), so the closure now clears the flag on its FIRST line or the veto would hide the grid for good on the way out.
  - **THE SEA** is `_swBuildHubWater`, handed to LSS by `LSSEarthWorld.buildWater()` - which only runs when the TILES are constructed. Kept world = never constructed = no ocean from round 2. Fixed with a **self-heal in `_lssGmapsTick` beside the sky's**, not in the keep branch, so it does not depend on who disposes the water or in what order. ⚠ **Rate-limited**: `buildWater()` stands up a ripple sim, a reflector and two flocks, and returns `false` forever on a level with no sea - 2 s after a success, **30 s after a failure**, and `_lssGmaps._seaRetryAt` is zeroed on every build so a new level never inherits the last one's back-off.
  - **Probes:** `__gridState()`-style scene scans are in the transcript; the live tells are the console lines `[lss-gmaps] KEPT the world over <level>` (the broken path ran), `[lss-gmaps] sea vanished - rebuilt` and `[lss-gmaps] weather vanished - re-initialising`. `window.__earthKeep = false` forces the rebuild branch, which is the only in-session way to exercise `cleanup()` without leaving the map.
- **(v47.16) THE CAPTURE RING IS FRIEND-OR-FOE, NOT FLEET IDENTITY (`drawCircumpunctHUD`).** The champion-capture circle around the crosshair was coloured `_cc.isA ? red : green` - v39.56's *"the claiming fleet's colour (red A / green B)"* - so **a Fleet A player watched a RED ring fill while capturing their OWN objective, and a Fleet B player got green for the enemy taking theirs.** Both readings were inverted for exactly half the room, which is why it survived: whoever checked it was on Fleet B. Owner: *"if friendlies are capturing an objective, it should be a green circle on the HUD, but if an enemy is capturing it should be a red circle"*. **The fix was to read a field that already existed** - `_champCaptureState` has returned `mine` since v39.55 and **nothing had ever read it**. ⚠ Spectating is the one case with no friend or foe (`player.team` really can be unassigned - the arena lineup defaults it to A for that reason), so it keeps the old fleet colour rather than calling everything hostile. ⚠ The DOM bar (`#champ-capture`, `.cc-a`/`.cc-b`) is still fleet-keyed **on purpose**: it is hidden unless `window.__champBar = true` and its label says FLEET A / FLEET B, so there the colour is an identity readout, not a threat readout. **Verified by pixel count off `#circumpunct-hud`, all four combinations, 1617 arc px to 0:** mine-A/cap-A GREEN, mine-A/cap-B RED, mine-B/cap-B GREEN, mine-B/cap-A RED.
- **(v47.17-47.19) NRG + AMMO ARE TURNED PLATES, NOT CURVED CAPTIONS (`_hlfFlatPlate`).** Owner: *"put NRG text and AMMO text in the same orientation at the dash dots (rotated from what it's at), and remove the number readout for each"*. `_hlfLabelPlate` rides its text along the bar, which is right on the 136-degree top rows but means the 44-degree FLANK bars read almost **vertically** - the tangent at NRG/AMMO is ~115 degrees. The dash pip row in the same band is a straight strip at `_HLF_DASH.rot`, so both labels now take their tilt **from that constant** and cannot drift out of step with the thing they were told to match; they are MIRRORED (NRG `-`, AMMO `+`) the way the HUD already mirrors its flanks (`_HL.ab1.rot` -50 / `ab2` +50). ⚠ **The PLATE turns with the text** - rotating only the glyphs inside an arc cutout puts a ~1.4vm word across a 1.5vm cell at 85 degrees to the cell's own axis: it fits by arithmetic and reads as a mistake. Numbers dropped, which retired `plateDeg` (its only job was fitting a plate to a string whose length changed with the reading) and **leaves NO numeric ammo readout on the framed HUD** - deliberate, same call as the v35.42 note for hull.
  - **⚠ v47.18 REVERSED v47.01's "flush with the bar's OUTER EDGE" rule for these two.** That rule was right for an ARC cutout, where sharing the bar's edge is what made it look punched through; a plate turned off the bar's axis shares no edge with it, so the same offset just reads as misplaced. It showed up on AMMO and not NRG **because the two bands are different depths** - NRG is the band's inner half (11.2..13.2) so an outer-edge offset happened to land near the middle anyway, while AMMO spans the whole band (11.2..15.2) where it is 1.25vm off. Owner: *"NRG fits, AMMO is a off center in its bar"*. Each now takes its own bar's MID radius, which also puts AMMO at 13.2 - the dash row's own radius.
  - **The angular anchor is the end the bar READS ZERO from**, i.e. the end its fill grows from: NRG lights from `a0` (152), AMMO is `fromEnd` so it lights from `a1` (28). Owner: *"NRG can actually lower to the bottom of its bar" / "by bottom i mean where the bar would read 0"*.
  - **⚠ The HUD cannot be magnified with a CSS transform for inspection** - `_lssApplyShipRig` rewrites `hudCanvas.style.transform` every frame (the v35.03 parallax), so a scale set from the console is wiped on the next tick. `input.hudScale` caps at 1.75, and vmin sizing makes the HUD **scale-invariant to the viewport**, so `resize_window` buys nothing either. What works: `input.hudScale = 1.75` plus reading `#circumpunct-hud` pixels directly. **Measuring beats screenshotting here** - collect the near-white text pixels (`rgba(236,247,255,.95)`) in a window and take the principal axis for the tilt, the centroid for the position. Measured: NRG tilt **-31.1** at 152.4 deg / r 12.24, AMMO **+30.8** at 27.6 deg / r 13.11.
  - **Live knobs `window.__hudLbl = { slide, radial }`** (defaults 0.5 / 0.5 in `_HLF_LBL`): `slide` is how far past the zero end the plate hangs as a fraction of its own width (0 fully inside, 0.5 centred on the end, 1 clear of the bar), `radial` is the same across the band (0 inner edge, 1 outer). Added because this is eye-tuned against a 12-pixel word and a rebuild per nudge is the slow way to find a number nobody can derive.
  - ⚠ **The HUD lab (`hud_concepts.html`) does NOT view this.** Its extractor lifts `_HL` + `_hlDrawHUD` from `lss.js`; the shipped framed assembly is `_HLF` / `_hlfDraw`, which the lab's ORIGINAL and VARIANTS tabs do not carry. Tune `_HLF` in the GAME.
- **(v47.20) THE ROLE ICONS GREW OUTWARD, AND THE READY-POP STOPPED EATING THE NRG BAR (`_HLF_ICON_ROW`).** Owner: *"make the 3 icons slightly bigger, but don't make them overlap the NRG bar, they can expand outward"*. 1.85 -> **2.10vm**. The row is no longer CENTRED in the band's outer half: it is pinned by its **inner edge** (`rIn` = NRG's outer edge 13.2 + `GAP`) and every extra vmin travels outward, so size and the no-overlap rule stop fighting. At rest it reaches 15.50, past the 15.2 band edge - **free, because `_HLF.FR` only spans `TOP` 200..340 while this row sits at 152..196**, and the old `labR` 16.3 captions are long gone. ⚠ `GAP` is **0.20, not 0.10, because it has to cover the STROKE**: `_hlfIcon` strokes at `vm*0.17` centred on the outline, so the drawn edge reaches 0.085vm inside `rIn`. (The ready GLOW is a `shadowBlur` halo, not an edge - it washes over the bar by design and always did.)
  - **⚠ THE READY-POP WAS ALREADY OVERLAPPING, before any resize.** `pop` scales the icon up to **1.32x about its CENTRE** for 0.45 s, which put the inner edge **0.34vm inside the NRG bar** every time an ability came off cooldown - invisible as a bug because it is brief and looks like part of the flash. It now grows from the inner edge instead (`rDraw = IR.rIn + szV * pop / 2`), which makes the clearance **exact for any pop by cancellation**, not approximately right. `dPerStep` keys off `IR.rIn + szV/2` so the row's angular spacing tracks a live size change.
  - **Measured** by A/B-ing the rendered pixels through the live knob: lit samples in the 12.4..13.40 polar band next to NRG were **2651 at size 2.10 vs 2648 at size 0.40** - 0.1%, i.e. antialias noise plus the control's own stroke. The big icons add nothing beside the bar.
  - **Live knob `window.__hudIcon = { size }`**, same argument as `window.__hudLbl`: eye-tuned against a bar it must not touch.
- **(v47.21) THE FLAT-SCREEN HUD SHIPS AT 1.75x, THE TOP OF ITS SLIDER (`HUD_SCALE_DEFAULT`).** Owner: *"let's max out the HUD size by default in the settings (1.75x)"*. ⚠ **SIX places spelled this `1`** and only ever agreed by hand - the `input` default, `_hlScale`'s fallback, the settings page's build-time markup AND its reopen path, `saveSettings`, and `SHIPPED_DEFAULTS` - so they are one constant now, declared immediately above `const input` because `input`'s own initialiser reads it (v39.02 TDZ: a later top-level const read from an earlier initialiser throws, and `typeof` does not shield it). ⚠ **A SAVED SETTING STILL WINS.** `loadSettings` writes `data.hudScale` over the default whenever the key is present, which is every save written since v36.19 - so this changes FIRST BOOT, RESET TO DEFAULTS, and pre-v36.19 saves that have no key. **Anyone who has already moved the slider keeps their number: a default is not a migration.** Verified all three: no save -> 1.75 (and `SHIPPED_DEFAULTS` seeds 1.75 into storage); a save holding 1.0 -> **stays 1.0**; a save with the key deleted -> 1.75; settings slider reads `value 1.75 / max 1.75`, display "1.75x".
- **(v47.22) CORE IS A CHEVRON RUN (`_hlfChevronRow`).** Owner: *"i want the core bar to look unique... maybe like a sideways chevron pattern instead of a solid line"*. CORE was the third `{ solid: true }` `_hlfBarRow` in a row of them, so the gauge answering *"can I fire the big thing"* looked exactly like SHIELD. It is now a run of arrowheads pointing the way the meter FILLS (apex at the LARGER angle, because CORE grows `a0`->`a1`), which reads as a charge rather than a level and costs no extra colour. ⚠ **THE FILL IS CLIPPED, NOT QUANTISED** - lighting whole chevrons would turn a continuous meter into a ~15-step one, and CORE is exactly the gauge you watch climb. Every chevron is stroked as an empty track, then the whole run is re-filled inside a **pie-sector clip** at the true fraction, so the edge lands mid-chevron. ⚠ Geometry is given in **ARC LENGTH (vmin at the mid radius), not degrees**, so the pattern keeps its proportions if the row ever moves radius. Chevrons survive the 0.85vm row where the v47.01 hatch did not, because a chevron is a **shape**, not a texture. Live knob `window.__hudCore = { pitch, thick, depth }`.
  - **Verified STRUCTURALLY, not by eye**: count separate radial runs of lit pixels per angle across the lit span - a solid bar is `1` everywhere. Measured **0 runs x70 (gaps), 1 x141 (apexes), 2 x88 (the two arms), 3 x2 (antialiasing)** over 301 samples at `corePct` 0.62. That test distinguishes a chevron from a bar without a screenshot, which matters because **the HUD cannot be magnified for inspection** (see the v47.17-19 note).
- **(v47.23) CORE AT READY IS THE HULL'S BRIGHTEST SHADE, NOT YELLOW (`sh.coreHot`).** Owner: *"the yellow for core is wrong, it needs to correspond to the brightest of the theme's color scheme"*. **CORE was already themed while CHARGING** (`sh.core`) - the yellow only appeared the moment it filled, which is why it looked like a bug rather than a missing feature. The cause: `coreState` conflated FIRING and READY, and the READY branch in the data layer baked a literal `_hlA('#f0ff1f', pulse)`. **A data layer reaching past the theme layer to pick a colour.** Fixed by making READY emit a pulse **ALPHA** (`v.corePulse`) and letting the draw apply it to a themed shade, and by splitting `v.coreFiring` out of `coreState` (which stays, because `coreReady` still reads it for the glow).
  - **`coreHot` is DERIVED, not typed**: the `_HLF_THEME` table's own maximum lightness **plus a step** (0.72 -> 0.82), at CORE's hue offset and saturation. So it stays the brightest shade in the cluster even if the table is re-tuned. ⚠ It lives OUTSIDE `_HLF_THEME` deliberately - an entry in there would raise `lMax` and chase its own tail.
  - ⚠ **FIRING keeps its un-themed amber `#ffbe3c`**, on the same rule as the ability amber and the hull ramp in the "deliberately NOT themed" list: an ACTIVE state has to mean the same thing on every hull.
  - **Verified by sampling rendered pixels** on TRACKER (hull hue 32): CORE-ready **hue 32 / L 0.82**, HP **32 / 0.46** (`health [0,1.00,0.46]`), SHIELD **46 / 0.64** (`shield [20,0.92,0.64]`, and 32 + 20x0.69 = 45.8). The hue-offset x SPREAD arithmetic checks out to within a degree, and CORE outranks every other row in lightness. ⚠ **Sampling gotcha**: the ready pulse composites at `I.ga x 0.75..1.0` ≈ alpha 138-183, so an `alpha > 220` gate finds NOTHING in that row; and a "brightest pixel" heuristic picks the near-white LABEL TEXT unless you require `max-min > 30` and skip the plate at 270 deg.
- **(v47.24-47.26) THE CHARGE SHOT AND THE CLUSTER MISSILE LEAVE THE GUNS (`_powerShotMuzzle`).** Owner: *"blaster's charged shot looks like it comes from his center mass... it needs to come from the guns, marked G1, G2... in the glb"*, then *"cluster missile, same thing"*, then *"blaster's charged shot needs to shoot from all the gun spots at the same time"*. **The markers are `gun1..gunN`** (`/^gun\d+$/i` in `buildModelShipMesh` -> `userData.muzzleNodes`); G1/G2 is the owner's name for them. ⚠ **`blaster.glb` carries EIGHT**, puncture/pyro/tracker one each, slayer/syphon/vortex two - check before assuming two.
  - **`firePowerShot` spawned every tracer, pellet and net broadcast at `player.position`** while the CHARGE has sat on the markers since v37.68 (`_blasterChargeGlow` parents an orb to each). The weapon visibly charged at the barrels and fired out of the pilot's chest, which is what made it read as wrong rather than merely off. Long-range now fires **one beam per barrel, all converging on the same `end`** (the pale companion beam stays on barrel 0 only - one per barrel is 16 tracers for a single trigger pull); close-range cycles its 12 pellets around the rack.
  - **The Cluster Missile was a SCREEN FRACTION.** v30.82 moved it off the camera ("from my face") by launching at `_computeScreenMuzzleWorld(0.644, 0.638)` - the upper-right of the **painted cockpit art**, correct for a 2D overlay and meaningless once v13 cockpit hulls carried real markers, in third person, or whenever that call DECLINED and it fell back to `player.position`. Marker first now, screen fraction kept only as a fallback. The BOT's cluster missile spawned at `this.position` too, while its own Stun Bolt three lines below already used the markers.
  - ⚠ **COSMETIC ONLY** (the v32.21 rule): wall raycasts, hit tests and the water cross still run from `player.position`. ⚠ **`_adsAnchor` for instant visuals, NOT for projectiles**: the v39.77 anchor slides a tracer onto the barrel as DRAWN under third-person zoom, but a projectile persists and flies in world space - anchoring would bend its whole trajectory. Tracers get it, missiles do not.
  - **Measured:** bot cluster missiles now spawn **42.2 / 24.3 u from the gun** vs **95.9 / 80.9 from centre** (marker sits 62.7 from centre; they aren't 0 because a 60 ms poll lets a 900 u/s missile travel ~54). Charge-shot origin landed **6.3 u from `gun5`, 101.7 from centre** (the 8 markers sit 104-109 out), and a long-range shot creates **9 new visible objects = 8 beams + 1 companion**.
  - ⚠⚠ **THE BROWSER PANE IS SHARED - THE OWNER MAY BE FLYING IT WHILE YOU MEASURE.** During this change the hull kept swapping between samples, the HUD vanished, `blasterMode` flipped back, and a cluster missile "spawned no projectile"; I attributed all of it to my own synthetic key events and wrote that up as a trap. **It was the owner at the keyboard** (*"i was the stray events"*) - including, almost certainly, the missing missile: pressing the ability key again while one is in the air is the v45.x AIRBURST, `_punctureDetonateClusters`, which removes it. **Unexplained state changes in the pane are a PERSON before they are a bug.** Say what you are about to drive, or take the reading from a passive watcher.
  - **Useful either way, and these are verified:** ability keys route through `_lssDispatchBound` off `input.kbBindings`, where **ability0 is `q`, not `1`** (the `1` in `input.keys` is something else), and the same dispatcher owns `v` (third person) and `[` `]` (**cycleHubShip**) - so a stray press from ANY source can swap the hull mid-measurement. Synthetic events do work: a dispatched `q` moved `abilityCooldowns[0]` 0 -> 7.7. **Driving STATE is still steadier than driving input**: Charge Shot is an ABILITY, so `player.powerShotCharging = true; player.powerShotCharge = 0.93` fires it straight from `updateAbilities`; for bot abilities, watch `game.projectiles` passively in a real match.
- **(v47.27) A REAL-WORLD LEVEL HAD NO CINEMATIC, BECAUSE IT HAS NO ROOMS (`_lssStartSpectatorCinematic`).** Owner: *"i did elimination in custom location... it didn't do a cinematic, or the cinematic was hidden or something"*. **It was not hidden - it never started.** The function bails one line in on `if (!Array.isArray(game.sdfRoomData) || game.sdfRoomData.length === 0) return false`, and `sdfRoomData` is the **marching-cubes ARENA's spawn rooms** - a streamed Earth level has **zero** (measured on Custom Location / Tokyo). **The lineup only ever needed two things from those rooms**: an ANCHOR POINT and a FORWARD DIRECTION. The roster, the spacing and the camera pose never touched them. So with no rooms it now anchors on the **team's own centroid** (`myShips` is already gathered and the ships are spawned by the time this hook runs - the live truth rather than a proxy) and faces the **enemy fleet's centroid**, falling back to the player's own heading when there is nobody to face. Verified `select|- -> warmup|- -> warmup|CINE -> warmup|- -> playing|-` with `sdfRoomData: 0`.
  - ⚠ **SCOPED TO GMAPS ON PURPOSE** (`_lssGmaps.active && !overlayOnly`), not "any level without rooms": the **OVERWORLD also has no `sdfRoomData`** and also declines here (see the v37.83 cyberpunk note), and whether the hub should gain a lineup is a separate question nobody has asked.
  - ⚠ **RACE IS EXCLUDED ON EVERY MAP, one line earlier** (`if (LSS.MODE === 'race') return false`) - so "race mode too" is by design, NOT a Custom Location bug. Unchanged here.
  - ⚠⚠ **TESTING TRAP THAT COST A FALSE "CONFIRMED"**: launching Custom Location **without typing a location** leaves it stuck on the *PREPARING ARENA* loading overlay forever - `game.state` sits at `warmup`, `__earthTiles` exists and `sdfRoomData` reads 0, so it LOOKS like a loaded level that declined the cinematic. It has not loaded at all. **Set `#gmaps-loc-input` and click `#gmaps-loc-go`, and wait for `#map-window-name` to show the resolved place**, before `#ship-preview-confirm` / `#ship-preview-launch`. Check `#lss-loading-overlay`'s computed display before trusting any in-level reading.
- **(v47.28) THE CINEMATIC WAS PLAYING BEHIND THE LOADING CURTAIN (`_lssRunCinematicThen`).** Owner, after v47.27: *"i didn't see a cinematic"* - and they were right, twice over. v47.27 made it START on a real-world level; this makes it VISIBLE. **`hideLoadingOverlay()` being CALLED is not the same as the curtain being DOWN**: on any gmaps level that call HOLDS and retries until the world is built (its v46.87 branch, so the player does not arrive to pop-in), but every launch path calls it and starts the cinematic on the very next line. **Measured over Tokyo: `CINE | overlay:flex` for the entire flight, curtain down only afterwards** - so what the player saw was a loading screen and then the 3-2-1. An ARENA never showed it because there the curtain drops on the first call. Now the solo paths wait for `getComputedStyle(overlay).display === 'none'`, bounded by `_CINE_CURTAIN_MAX_MS` **50 s - deliberately longer than the earth curtain's own 45 s ceiling** so the curtain always lifts first, and past it the flight plays anyway rather than stranding the 3-2-1. Verified: Tokyo waited **3.9 s**, a cold Osaka **19.4 s**, both then `CINE | overlay:none`.
  - ⚠⚠ **THE MULTIPLAYER CALLER OPTS OUT (`waitCurtain: false`), and must.** In `scheduleLaunch` the cinematic plays inside a FIXED window and does NOT own the 3-2-1 (`_lssArmSyncedCountdown` fires on a shared absolute deadline), and that path lowers the curtain only `if (_cineOn)` - i.e. only if the call actually STARTED a flight. A deferral there reads as "declined", so the curtain stays up AND the cinematic lands on top of the countdown. Nearly shipped as a regression; caught by checking the other call sites before trusting the solo fix.
  - **On "the match should start at the same time for everyone" (owner):** that half is already done and has been since v45.16 - the lowest peerId proposes an absolute `launchAt`, `_lssArmSyncedCountdown(_cdAt)` fires on a shared deadline, and the cinematic was deliberately decoupled from it. **What the handshake does NOT include is WORLD readiness**: the gate is `peer.warmupReady`, which means SHADERS warm. A gmaps world builds per-device and its curtain can hold for tens of seconds, so a slow peer can still be behind its own curtain when the shared 3-2-1 fires. Closing that means adding a curtain-down signal to the readiness handshake and gating the `launch_at` proposal on it - **with a ceiling**, or one peer that never reports ready strands the whole room.
  - ⚠⚠ **`game.entities` ON AN EARTH LEVEL IS MOSTLY CITY TRAFFIC, NOT BOTS.** Measured in elimination over Tokyo: **30 entries = 5 real bots + 25 ambient ships** carrying `earthKind` on `team9300`. Those 25 spawn and accelerate **during the 3-2-1**, so any "are the bots moving during the countdown" probe that counts `game.entities` reads a steady climb (2 -> 24 moving) and looks exactly like bots breaking formation. **Filter on `e.loadoutKey`** - only real combat ships have one. With that filter, bots measure `maxV: 0` and zero drift for the whole countdown on BOTH a normal arena (`hourglass`) and Custom Location, and are released only at `playing` - i.e. the v47.28 launch is correct and the moving ships are the city waking up.
  - **The solo sequence now matches the owner's spec** - *"loading screen, sync, curtain lift, cinematic, countdown, start"* - as `ov:flex` -> `ov:none + CINE` -> `wt 2.8..0` -> `playing`. **`sync` is the one beat still missing, and it is the multiplayer item above.**
- **(v47.29) `warmup_ready` NOW MEANS "MY WORLD EXISTS" ON AN EARTH MAP (`_lssEarthWorldPending`).** Owner: *"the cinematic should really be sync'd so it starts when all the players are loaded... then the match starts at the same time for everyone"*. ⚠ **Most of this was ALREADY BUILT and I first described the gap too broadly.** v45.12 ("READY HAS TO MEAN LOADED") already routes the announcement through `_cineWhenSettled` and gates the launch on `_allPeersWarmupReady()`; v45.13 already tuned the cap to 120 s after a PC+PHONE room. **The single missing term was the gmaps world build**: `_cineHeavyWorkPending` covers shader warmup, hub preload, world prebake, `_PREBAKE.on` and swap/round staging - all arena/hub work - so a real-world peer satisfied every check it knew about while its patches, its spawn placement and its city were still arriving.
  - **The fix is an EXTRACTION, not new logic.** The only code that knew whether an Earth level had finished building was the six-term condition inline in `hideLoadingOverlay` (`_noWorld || _noSpawn || _building || _regionUnknown || _cityPending || _noCircuit`). It is now `_lssEarthWorldPending()`, called by BOTH the curtain and `_cineHeavyWorkPending`. ⚠ Moved **verbatim** - same terms, same order, same short-circuit - because two copies would drift and the notes inside it are the record of what each term cost to learn. ⚠ **The curtain's own `_LSS_EARTH_CURTAIN_MAX_MS` deadline is INSIDE the predicate**: past it the curtain gives up and flies, so "pending" has to go false at the same instant or a peer whose world never completes holds the room to the 120 s ready cap instead of the 45 s the curtain actually waits.
  - **Measured, two tabs, Custom Location over Tokyo** (wall-clock delta between HOST and PEER): loading screen **25 ms**, curtain lift + cinematic **313 ms**, cinematic end **204 ms**, countdown start **2 ms**, `playing` **12 ms**. Solo regression on the same map: `select -> loading 10 s -> CINE 5.5 s -> countdown 3 s -> playing`, i.e. the owner's full sequence.
  - ⚠⚠ **A SAME-MACHINE TWO-TAB TEST PROVES THE PROTOCOL, NEVER DEVICE DIVERGENCE** (the v45.13-45.16 lesson, and it still applies): both tabs build at the same speed, so the case this change exists for - a phone at 40 s against a PC at 8 s - is NOT exercised by the numbers above. What the test does prove is the regression risk: the new term did not deadlock the gate.
- **(v47.32) THE LINEUP IS THE STARTING SPOT - and it unified THREE reports into ONE bug.** Owner, in order: *"the bots disappeared"*, *"my ship turned 90 degrees after the cinematic"*, *"they shouldn't move after the cinematic... no ship should... that's its starting spot"*. ⚠ **THE CINEMATIC IS MESH-ONLY**: it poses `mesh`, never `ent.position`, re-applies that pose every tick, and `_lssCompleteSpectatorCinematic` then snaps every mesh back to `origPos/origQuat`. **v39.95 had already fixed this for the PLAYER ALONE** (the last 1.4 s blends their ship to the gameplay pose and overwrites its `orig*`) and the note beside it concedes the rest - *"the teammate snap at the cut happens off-axis behind the camera"*. It is not off-axis when the lineup is your own fleet: they snap back to spawn kilometres away, which reads as **vanishing**. The **90-degree turn** is the same snap on your own ship whenever that blend does NOT complete - in multiplayer the flight runs inside a fixed window and can be cut short, skipping the overwrite entirely.
  - **Fix: commit the slot to the ENTITY at lineup time, and set `orig*` to the slot** - then the blend target, the gameplay pose and the restore all agree and there is nothing left to snap. A mesh pose alone does not stick: `Bot.update` runs `mesh.position.copy(this.position)` every frame, and `mesh.lookAt` only at speed > 1, so committing POSITION is enough and the slot orientation survives on its own until the ship moves.
  - ⚠ **CLEARANCE IS WHY THIS IS NOT A ONE-LINER.** Lineup slots are laid out geometrically around the spawn anchor and are NOT validated against the world - on a real-world level that can put a ship inside a tower, the exact hazard `tiles.spawnPoints()` exists to avoid. Each slot is scored with `_spawnClearanceScore` first, and a slot that does not clear leaves THAT ship on its own validated spawn. One ship out of formation beats one ship inside a building.
  - **Measured over Tokyo, the two beats (cinematic end -> playing):** player moved **0**, player turned **0 deg**, bots moved **0/0/0/0/1**; teammates sat at **196 u** (in formation) and enemies at 8,848 / 12,476 / 13,458 at BOTH beats.
  - ⚠⚠ **DO NOT MEASURE FORMATION LONG AFTER LAUNCH.** Reading teammate distance ~35 s into a live match showed 2,837 / 4,791 u and looked like the fix had failed - the bots had simply been FLYING for half a minute. Snapshot at the beat you care about.
- **(v47.33) THE PEER'S SPAWN SHIELD, AND THE CURTAIN ON THE SHARED INSTANT.** Two owner reports from the same launch.
  - **"we could see an activated shield around the other player during the start, but then it hid as it should".** That bubble is `NetworkPlayer`'s SPAWN-PROTECTION visual (`s.spawnProt > 0` -> `setShieldIntensity`). ⚠ **This is v36.73's complaint one seat over**: that pass removed the bubble from the LOCAL ship ("starting a match in third person shows the shield for a second, it shouldn't show it at all") and DELIBERATELY kept it on peers and bots, because *"protected, don't waste shots"* is a real multiplayer tell. That reasoning also says exactly when it is worth drawing: **it is a tell about SHOOTING**, so it has no job during the cinematic and the 3-2-1 when nobody can shoot - it just lights a dome on the one ship you are being shown. Gated on `game.state === 'playing'`. ⚠ Gate the SOURCE, not the mesh: the cinematic hides `shieldMesh` for ships in its OWN lineup, and an enemy peer is never in yours, which is why nothing covered this.
  - **"the sync was close, like about a second apart on the cinematic"** while the countdown measured 2 ms. Both peers reach `scheduleLaunch`'s timer at the same wall-clock instant, but it then called `hideLoadingOverlay`, whose gmaps branch re-asks `_lssEarthWorldPending()` on its own 16 ms retry against its own frame pacing - and the cinematic is only VISIBLE once your own curtain is gone. That re-check is redundant there: since v47.29 the `launch_at` proposal is gated on every peer's world being built, and `launchAt` is a further 1.5 s after that, so the room has already answered the question. `game._launchCurtainFree` frees the curtain for that launch only, and is cleared on every gmaps build. **Measured, two tabs: cinematic start 313 ms -> 184 / 169 ms.**
  - ⚠⚠ **`playing` DELTA IS LOAD NOISE ON ONE MACHINE - DO NOT TUNE ON IT.** Across three runs it measured **12 / 1072 / 108 ms**. The peer tab ran at **43 fps against the host's 83** (two full game instances on one box), and the warmup->playing arm carries its own per-peer guards (staging, arena mesh build) on top of the wall-clock `_anchorTimer`. A single bad sample here looks exactly like a regression and is not one - take a second sample before believing it.
- **(v47.30-47.31) GMAPS TEAM SPAWNS: TWO ENDS, AND THE FLEET ACTUALLY MOVES TO THEM.** Owner, two peers over Tokyo: *"they spawned right beside each other even tho opposite teams"*. Only the ASSAULT branch ever tagged `q.team` on the gmaps spawn pool, so elimination/classic reached `getValidSpawnPoint` untagged; worse, the non-assault placement was `const _s0 = _sp[0]` - **the same single point for everyone**, run locally on every peer. ⚠ Split by BEARING (origin -> furthest point), never by radius: the points are concentric RINGS, so a radius split puts two shells on one centre (the assault note measured 33 u apart). ⚠ **And CLUSTER each end** - the first cut took the top 35% by projection, which separated the ENDS correctly (8,479 u) while each end internally SPANNED 9,557 / 9,616 u, so your own wingmen were 3,664 and 8,109 u away. Take the N points nearest the extreme point instead. ⚠ **v47.31: re-place the BOTS too.** v46.89 re-picks the PLAYER once the pool is real ("the first pick happened before this world existed") - equally true of every bot, and only the player was covered, so the player teleported to their end and the fleet stayed at the pre-build origin. Splitting the ends without this makes the symptom WORSE, which is how it surfaced.
- **Perf companion:** [`performance_review.md`](performance_review.md) — the **measured** baseline for all 8 modes (loading timelines, prebake phase splits, JS self-profiles, per-pass GPU times, multiplayer), taken on v47.01 on 2026-09-20/21. **Read it before starting any perf work**: it has the per-mode load table, the ranked fix list, a section of *negative* results (things already measured NOT to be problems — round transitions, monster prime, the 13 s build frame), and the harness to reproduce any of it. Two rules it established: `__prebake()`'s phase attribution is misleading on hub maps because every phase is priced in yielded GAME FRAMES, and a hub loading number read off the Browser pane understates the real one by ~3x.
...
6. **(v47.02) THE COROLLARY OF RULE 5: A WARM THAT BINDS THE WRONG TARGET IS PURE WASTE, AND IT COMPOUNDS.** `_warmRealCombatFXInner` used to compile AND draw the scene three ways — canvas at `NoToneMapping`, canvas at the live `ACESFilmic`, then `postFX.rtScene` — because a 2020-era comment assumed a direct-to-screen path existed. It does not: every live scene draw binds a target (`renderPostFX` -> `rtScene` on **both** branches, cineFX -> the composer RT, XR -> `xr.getRenderTarget()`), and `renderFrame`'s potato branch is unreachable because `applyQualityPreset` maps `'potato' -> 'low'` on its first line. **Measured: ~128 of the ~193 programs that phase created were dead; cyberpunk went 451 -> 331 programs at the curtain lift and its prebake 7,736 -> 2,409 ms.** The compounding is the lesson — `ms.carrier` 3,842 -> 195, `ms.cloak` 2,518 -> 112, `ms.drainCloak` 2,348 -> 1, because those phases were linking and draining the dead canvas variants of every hull. lss.map.md's own v39.81 water entry had already named this class of waste and left it open. ⚠ Two traps if you touch it again: the prebake's `drainFx0` drain must stay **unconditional** (it used to be gated on the finisher this change stops returning), and the `__fxWarmProgs` namer must sit BELOW the surviving compile or it names nothing. If a direct-to-screen scene path is ever revived, warm it there with ITS OWN target bound — toggling `renderer.toneMapping` alone never reproduced the combat key, which is why v36.17 added the `rtScene` bind.
7. **`_clip` name collision.** `_clip*` at **~L22110** = the *terrain clipmap* LOD system (`_clipGeo`/`_clipBuild`/`_clipUpdate`/`_clipmap`). `_clip` at **~L46572** = the *video recorder* (MediaRecorder). Totally unrelated — grep with care.
8. **(v47.02) `_stHash2i` JOINS THE SYNCED SET — and there are FOUR copies of the terrain math, not three.** `_stNoise2` now reads its 2x2 integer lattice corners through a direct-mapped cache `_stHash2i` (right under `_stHash2`) instead of calling `_stHash2` four times. It is **value-preserving by construction** — on a miss it computes the identical expression — and that was proven three ways: the height bake's own 48-point worker-vs-main-thread self-check, 5.96 M samples compared in node with **0 differing** and `Object.is`-equal float sums, and the unchanged 17,831,061-eval count. **Measured: the cold hub bake 16,064 -> 9,119 ms, `ms.clip` 12,980 -> 6,096, and the hub prebake stopped hitting `_PREBAKE_MAX_MS` — so the entity prime, the hull/cloak warm and the 14-pass GPU prime, all of which the hub had been silently SKIPPING, now actually run.** Its state hangs off the FUNCTION OBJECT, not a module `const`, so there is no temporal dead zone (rule: [`lss-tdz-const-trap`]) and `f.toString()` carries it into the worker strings. ⚠ The copies that must stay in step are the worker-string copy, the main-thread copy, `worldSDF`, **and `_hbFamily()`'s array** — adding a function there changes `_hbHash`, which invalidates the 34 MB IndexedDB height-bake cache exactly once. See [`performance_review.md`](performance_review.md).
9. **Triplicated terrain math.** `_st*` / `sdSphere` / `worldSDF` exist in **three copies** that must stay in sync: worker-string copy (~L17041), main-thread copy (~L17208), and the collision copy `worldSDF` (~L22496). Editing one without the others desyncs terrain vs. collision.
10. **Hub perf wins that stuck** (keep them): grass off by default, shadow map 2048², reflector throttle + RT shrink. Perf lives in `gameLoop`/`renderFrame`. **(v36.42)** the reflector throttle is now movement-aware (2..3 frames, not a flat 3) and the displaced water plane no longer renders into the reflection it samples — see PART 8's hub-water entries before touching any of it.
...
12. **Fragile seams — re-verify after any combat/render edit:** (a) combat first-sight hitch, (b) hub framerate, (c) multiplayer combat. The sandbox renderer is useless for these; test in a real browser.
13. **(v36.29) NEVER hand-edit a `.glb` under `LSS/`.** Every shipped model is a BUILD ARTIFACT generated from `assets_src/` — see the Asset pipeline section below. Editing `LSS/**/*.glb` directly gets silently overwritten on the next run, and the pristine art only exists in `assets_src/`.
...
- **(v45.33-45.41) THE PICKER'S BOT HAND.** **Jump:** `_lssDealBotShips` / `_lssBotDealPrep` / `_lssBotSeats` / `_lssBotDealMine` / `_lssBotPickAllowed` / `_lssBotCycleShip` / `_lssBotDealAnnounce` · `makeBotSeat` inside `updateTeammatesStrip` · probe `__dbg.bots()`. `game._botShipDeal` used to be dealt inside `spawnBots()` (WORLD BUILD, i.e. after CONFIRM), so the picker had nothing to show; the deal moved to `_lssBotDealPrep` and `spawnBots` honours it through a **consumed** `fromPicker` flag. **⚠ A JOINER IS BRIEFLY THE BOT AUTHORITY** (`startElimination`'s join path seats the caller as host before `open_solo_start` lands), so an authority never adopts an inbound `bot_deal` - it replies with its own, rate-limited 3 s/peer. Any bot can be any ship (v45.34); both fleets are three seats and the tally counts the human at this keyboard (v45.41); the wheel-to-change-hull gesture is gone - the arrows are the whole control (v45.41).
- **⭐⭐ (v45.43) REMOVE A SPECIFIC BOT — the ✕ / + on each fleet card.** Owner: *"should be able to click x on a bot's picture card to remove a specific bot, in case i want to do a 2v2 or 1v1, or 2v1 or 3v1 or whatever way / x will become a + to add it back in if needed."* **Jump:** `_lssBotSeatOff` / `_lssBotSeatIsOff` / `_lssBotSeatsOffIn` / `_lssBotToggleSeat` · the `chip-seat-btn` block in `makeChip` · `.chip-seat-btn` CSS · the `_seatOff` guard in `spawnBots` · the `_needB`/`_needA` correction in the ASSAULT reinforcement block · `eo`/`fo` on `bot_deal` · probe `__dbg.bots('enemy', 1, 'x')`.
  - `#btn-ss-bots` was already all-or-nothing (`game.eliminationBots`); this is the per-seat version, and it is what makes an **uneven** match reachable at all — 3v1 has no expression in a single toggle.
  - **⚠ THE FLAG IS KEYED BY SEAT INDEX, AND BOTH READERS SKIP RATHER THAN COMPACT.** `spawnBots` derives each bot's id from its loop index (`i + 1` enemy, `i + 10` friendly) and v45.34 made that id the key of the between-rounds `_prevStats` carry-over — so filtering the loadout array down would hand seat 2's running kills to seat 1 next round. **Verified: middle enemy seat off → bots spawn with ids 1 and 3, never 1 and 2.**
  - **⚠ IT LIVES ON `game._botSeatOff`, NOT ON `game._botShipDeal`.** The deal is re-shuffled on every visit to the picker (`_lssBotDealPrep`), and an owner who set up a 1v1 does not want it undone by starting the next match. Session-scoped: it resets on reload, and every card says REMOVED / OUT with a green **+**, so it can never be silently in effect.
  - **⚠ THE ASSAULT REINFORCEMENT TOP-UP HAD TO LEARN ABOUT IT** or it would put a deleted bot straight back every few seconds — the same shape of bug v45.41 fixed there for the 4v2. `_lssBotSeatsOffIn(side, n)` counts only inside the **bot window** (3 minus the humans on that fleet), because an off seat a human is already holding must subtract nothing.
  - **⚠ THE BUTTON IS NOT ON THE ART.** Owner, on the first cut: *"dont put remove right over the center"* — a bottom-centre pill sat squarely across the hull it was labelling. It is a 19px round glyph in the card's **bottom-right corner, on the text rows**. Everything else is spoken for: the top-right is the ready pip, and both left/right edges of `.chip-art` belong to the hull arrows for their whole vertically-centred band — `.chip-art` is 78/64/42px depending on breakpoint, so a 30px centred arrow reaches into every art corner at one size or the other. `.bot-seat .chip-name/.chip-ship` give up 27px of right padding so "VORTEX · REMOVED" never runs under it. **Measured at 1440×860 and at 820×420 (the phone-landscape breakpoint, 42px art / 74px card): no overlap with art or either arrow, no text overflow.**
  - **A removed seat keeps its card and its hull** — that card is the only way back, and the hull is the ship that returns when you press +. `.bot-off` greys the art, kills the arrows (a seat that is not coming has no hull to choose) and the pip reads OUT; the status line reads `<HULL> · REMOVED` through the v45.17 label/ship split.
  - **⚠ The ✕ needs the same `stopPropagation` the arrows do** — the enemy list's own delegated click is the v45.27 TEAM SWAP, so removing an enemy bot would otherwise also move you onto their fleet.
  - **Mirrored to the room as `eo`/`fo` on the existing `bot_deal`**, not a new event: the flags and the hulls are read together by every card, and a separate event could only ever arrive out of step with the hand it describes. Written positionally into the array `_lssBotSeatOff` hands out, never assigned wholesale.
  - **Verified live:** ✕→+ toggles the card and the flag; + restores it; **1v3 spawned exactly 3 enemy bots and 0 friendly**; **3v2 with the middle seat off spawned ids 1/3/10/11**; the removal survived a round transition (round 2 rebuilt the same four); zero console errors throughout.
- **⭐⭐ (v46.85) ADD BOTS IS DELETED — the fleet cards are the whole opponents control.** Owner: *"let's remove the 'add bots' button, since each player card has one anyway... then we can span the player cards top to bottom of the screen"*. **Jump:** `function _renderEliminationBotsBtn` (now a one-line stub) · the `(v46.85) #ss-bots-dock / #btn-ss-bots ... ARE GONE` CSS note where its skin was.
  - **The markup (`#ss-bots-dock`, `#btn-ss-bots`), its CSS, and the `.ss-no-bots-btn` collapse class are all deleted.** `_renderEliminationBotsBtn` is kept as a **deliberate no-op stub** — the same shape v35.20 left `updateReadyButton()` in — because it has **seven call sites and every one is behind a `typeof fn === 'function'` guard**, which is the file's signature silent-no-op trap: delete the declaration and those guards go quiet instead of erroring.
  - **⚠ NOTHING ABOUT BOTS CHANGED.** `game.eliminationBots` is still the all-or-nothing gate read at ROUND BUILD by all three spawn sites, `_setEliminationBots` still mirrors it onto `net.openSolo` and broadcasts, the `open_solo_start` / `elim_bots_off` peer events are untouched, and `startElimination` still seeds it from `_elimWantBots()` (default ON). Only the button that flipped it from the picker is gone — v45.43's per-seat ✕ / + is strictly more expressive anyway (it is what makes 3v1 reachable at all).
  - **⭐ THE BREAKPOINTS FLIPPED SENSE.** v36.30 hid `#fleet-scroll` at `≤1200` and `≤620h` and left a shrink-wrapped bots-only box, *because* the button had to stay reachable at every width. With the cards as the control, hiding them is what would leave no bot control at all — so both blocks now **keep the fleets**: `≤1200` narrows the column 210→168 (card floor 96, art floor 54) and moves the rails to `left:198`; `≤620h` only re-anchors (`top:74; bottom:10`) and scrolls internally. `max-width:900` goes **back to `display:none`** (pre-v36.30) — portrait has the right column top-anchored and the rails across the whole bottom edge, so there is nowhere to put a column — and the landscape-phone block, still last in the cascade, brings it back for the one short-and-wide case that fits.
  - **⚠ THE `≤620h` RAILS RULE HAD A STALE `left: 18px`** (set when the column collapsed there). **MEASURED at 1600×600 before the fix: `#teammates-strip × #ss-rails` overlapping 210×48.** `left` is now simply *not stated* in that block, so whichever of base (240) / `≤1200` (198) matched keeps its value — stating one would clobber the other, because `≤620h` is LATER in the sheet than `max-width:1200` and a 1000×600 window matches both.
...
Profiled per the owner's ask (`index.html?pbhud`, ELIMINATION on Custom Location over Toronto). **The flight itself is clean** — worst frame 16.8-41.6 ms, blame `renderFrame`, nothing streaming-related. **The whole hitch is one frame at the end of the load.**
- **What it is.** One frame creates **14 programs, 870 geometries, 13 textures, 3.7 MB**, and takes **525 ms** (583 ms on a second run). `slowgl` names the blocking calls: `getProgramParameter` — the synchronous LINK_STATUS query — at **21, 21, 73 and 24 ms**. `first` names what pulled them in: `earth-terrain-0-36`, `earth-roofs-2`, `earth-walls-2`, `earth-ads`, and the life GLBs `Material_0.001` / `Material_0.002` / `mesh_0` (34k verts).
- **⭐⭐ THE WARM PASS RUNS, AND WARMS THE WRONG VARIANT.** The console timeline is decisive: `33.03 [earth-warm] 385 ms behind the curtain, programs 196 -> 210` → `33.13` **fourteen** `[cold]` links → `33.18` curtain lifts. The same materials link TWICE, and the probe prints why: `lambert [FORK: outputColorSpace toneMapping]`, `water [FORK: outputColorSpace toneMapping]`, `Material_0.001 [FORK: mapUv flags]`, `basic [FORK: flags]`, `bubbles [FORK: idx-52 shadowMapType]`, `lambert [FORK: idx-110 idx-67]`.
- **⚠ BINDING THE RENDER TARGET IS NOT THE FIX — measured, then reverted.** PART 13's rule 5 makes the bound target the obvious suspect, and `_lssEarthWarmOnce` is the one warm path in the file that never binds one (`_shaderWarmup` and `_warmRealCombatFX` both bind `postFX.rtScene`). Saving, binding `null` **and** `postFX.rtScene`, warming both and restoring **changed nothing**: same 14 cold links, same fork names, worst frame 525 → 583 ms, and the warm itself went 385 → 575 ms because it renders the scene twice. Reverted; the note stays in the function so nobody pays for that experiment again.
- **⭐⭐⭐ THE REGRESSION THIS PROFILING PASS UNCOVERED, WHICH MATTERED MORE THAN THE FRAME.** Owner,
  while it ran: *"i heard ships moving around during loading screen"*, then *"i think the match
  started before i got in it"*. **Both are v46.87's curtain, and both are one cause:** it made the
  loading screen wait 28-50 s for a gmaps world and told NOTHING ELSE. The launch sequence still
  flipped `game.state` to `'playing'` on its own schedule, so the match ran its whole opening behind
  an opaque overlay - round clock running, bots engaging, flyby voices locked onto ships the player
  could not see. **Jump:** `function _lssEarthCurtainUp` (the shared predicate) and its three users.
  - **(1) The match waits.** `_lssEarthCurtainUp()` joins the `warmupTimer <= 0 && !selectActive`
    guard in `updateRoundSystem`. This is *literally* the case the v36.58 Spire paragraph two
    comments above it already describes - a runtime world build the round must not start against -
    so it takes the same shape, ceiling and co-op caveat.
  - **(2) The flyby pool goes quiet.** The v36.24 LOADING-SCREEN AUDIO HOLD lives inside `playSound`,
    and **a flyby voice is not a `playSound` caller** - `_flybyAssign` wires a BufferSource straight
    to `audio.sfxBus`. Invisible while a curtain lasted two seconds; not at thirty. `_flybyTick`'s
    `_live` now reads the same `_loadingAudioHold` flag, and the existing no-candidate sweep releases
    whatever was running.
  - **(3) No population behind the curtain.** `_lssEarthLifeTick` is skipped while it is up: the
    carrier brings its OWN audio graph (`_carrierEngineStart`, another non-`playSound` voice), and
    spawning a city's traffic during a 30-50 s load also meant arriving to ships that had already
    flown half their route. The PATCHES still stream - the world is what the curtain waits for.
  - **MEASURED, cold Lisbon, elimination:** curtain up **21.12 → 50.86 s**; `statesWhileUp` is
    **`["warmup"]`** and nothing else; the round timer sat at its full **80 s** the whole time and
    started counting at the lift; **zero** active flyby voices throughout.
    ⚠ `window.__flybyDbg()` returns the STRING `"no active flyby voices"` when idle - `.length` on
    it reads 22 and looks like a voice count. Check the type.
- **What the evidence pointed at, and it was right.** `toneMapping`, `outputColorSpace` and `shadowMap.type` are all in three's program cache key and are all **renderer globals**, not per-draw state. `_lssEarthWarmOnce` is called from `hideLoadingOverlay` on a `setTimeout`, i.e. **outside the render loop**, so it sees whatever the last pass left those at while the first real frame sets its own. **Warming THROUGH `renderFrame`** (which sets that state itself) cut the forks from 14-15 to 5, confirming the diagnosis. `window.__earthWarmLegacy = 1` restores the old warm at runtime for a one-session A/B.
- **⭐⭐ AND THE BIG FRAME WAS NEVER VISIBLE - the first reading was wrong.** That run recorded the 525 ms frame but not the curtain lift, and "a frame 11 s after the cold links must be in play" was an assumption, not a measurement. **Across four runs - warm AND cold cache, legacy AND fixed warm - `bigAfterLift` is EMPTY every time**, all 20 big frames behind the curtain (cold Osaka, legacy: lift 50.67 s, worst 416.7 ms at 49.84). So the `renderFrame` warm buys duplicated WORK, not smoothness: nine or ten programs per load stop being built twice. Worst frame is unchanged within noise (466.6 / 475.1 / 416.7 ms) because **the big frame is not the links** - the same frame creates ~850 geometries and 3.7 MB, which is v46.87's `_leDeferPublish` fix working as designed.
- **⚠ PROCESS: record the CURTAIN LIFT beside any load-time frame number.** A `MutationObserver` on `#lss-loading-overlay`'s class costs nothing, and without it a big frame reads as a stall when it is just work happening behind a loading screen.
- **Also seen and not chased:** the prebake line reads `[prebake] classic/gmaps_user 1598-1765 ms ... [mon 306-747, carrier 557-656, cloak 346-604]` — monsters, carrier and cloak are most of a ~1.7 s prebake on a map that has no hub monsters.

#### ⭐⭐⭐ (v46.87) CUSTOM LOCATION *IS* EARTH — and the loading curtain belongs to the WORLD, not the MODE
**Jump:** `MAP_DATA.gmaps_user` (gained `stream: true`) · the `k === 'gmaps_earth'` line in `_visibleMapKeys` · `function _lssEarthCurtainArmed` (above `hideLoadingOverlay`) · the `_streamed` guard on `_regionUnknown`/`_cityPending` · `function _leDeferPublish` · the `else if (typeof t.publishTick === 'function')` arm in `_lssGmapsTick`.
Owner: *"in other modes like elimination, that have custom location or earth custom location, the warmup/bake/prebake/loading screen doesn't wait for the buildings to be drawn like in free flight: earth"*, then *"earth and custom location should be the same thing"* and *"all the work we did on earth mode, that's the custom location we'll use"*.
- **THE TWO WERE DIFFERENT ENGINES, not two presets.** `gmaps_earth` ("Earth", free flight) carries `stream: true`, so `_lssGmapsBuildLevel` builds **`LSSEarthWorld`** — patches streamed around the player on one projection origin, the Overpass region + backfill, the publish budget, night lights, traffic and monsters. `gmaps_user` ("Custom Location", classic/elimination/race) had no `stream`, so it built **`LSSEarthTiles`**: ONE fixed 2.6 km patch and none of the above. Every piece of Earth work since v45.47 went into the streamed world and none of it reached the slot the other modes actually use. `gmaps_user` now streams; the two entries were already identical in every other field (`scale: 7`, `patchMetres` defaulting to 2600).
- **ONE ENTRY OUTSIDE FREE FLIGHT.** `_visibleMapKeys`'s final filter now drops `gmaps_earth` — the freeflight branch returns before it, so that arm is untouched by construction. Listing both was listing one map twice under two names.
- **THE LAUNCH REFUSAL RETIRES ITSELF.** `commitLoadout`'s `_v12mValidate` allowed a gmaps map with no typed DROP location only when `_selfSited` (`stream` **and** own lat/lng) — now true here, so *"Custom Location selected but no place typed"* is gone and LAUNCH drops you on the stored coordinates. A typed DROP still wins: the override in `buildRoomGraphLevel` writes `level.lat/lng` before the build, for any `type === 'gmaps'` level.
- **⭐⭐ THE CURTAIN WAS ARMED ON MODE FLAGS.** `hideLoadingOverlay`'s Earth hold asked `game._earth.armed` (the FREE FLIGHT: EARTH tag) and `_isEarthCircuit()` — both **modes**. What it waits for is a **world**: a DEM, patches, an Overpass region, an extruded city, none of which care which menu button started the match. `_lssEarthCurtainArmed()` asks the LEVEL instead, and deliberately liberally — `game.currentLevel` **or** `MAP_DATA[game.selectedMap]` — because those two disagree for exactly the window this covers: `currentLevel` is written at the top of `_lssGmapsBuildLevel`, and before that runs only `selectedMap` knows a gmaps world is coming. The 45 s ceiling bounds a wrong answer.
- **⚠ `_bldRegionKey` / `_bldFail` / `stats.regionWays` ARE `LSSEarthWorld` MEMBERS.** On `LSSEarthTiles` they are absent, which reads as *"region unknown and Overpass has not failed"* — true forever — so widening the arming without a `_streamed` guard would pin the curtain to its 45 s ceiling on a finished world. The single patch is covered by `_noSpawn`, which follows its own `await tiles.ready`.
- **⚠⚠ TWO LATENT BUGS FOUND ON THE WAY, both about meshes reaching the scene:**
  - **`_leDeferPublish` looked up `loading-overlay`.** There is no such element — the markup and all eleven other lookups use **`lss-loading-overlay`** — so the "publish IMMEDIATELY while the curtain is up" escape its own comment describes had never once fired. From the moment the launch sequence flips state to `'playing'` (measured ~6.5 s on a cold world, curtain up for seconds more) the city was drip-fed at 4 meshes a frame *behind the loading screen*. Also switched to reading the **`active` class**: the overlay's base rule is `display:none`, so a computed-style test calls a raised curtain lowered.
  - **A single-patch level's publish queue had NO DRAIN.** `_publish` queues into `_pubQ`, and the only `publishTick` caller in the file is inside **`LSSEarthWorld.streamUpdate`** — which `LSSEarthTiles` does not have. Any chunk built after state went `'playing'` was queued and **never added to the group at all**: not late, never. That is the other half of "the buildings aren't drawn". Custom Location streams now so the branch is reachable only via `window.__earthStream = 0`, but a queue with no consumer is a landmine either way — `_lssGmapsTick` drains it.
- **Verified live, four launches, zero console errors:** solo/elimination on **Custom Location** — curtain held **0.31 → 30.16 s** on a cold New York and lifted onto a built city (`LSSEarthWorld` in the scene, 826 nodes at the lift and climbing); **Chicago via a typed DROP** — level retargeted to 41.8756/-87.6244, held to **28.26 s**, **965** nodes at lift, 1,194 meshes, city visible from altitude; **The Nexus** (a normal arena, the regression that mattered) — lifts at **2.98 s**, unchanged; **FREE FLIGHT: EARTH** — still its own `gmaps_earth` entry, Toronto with the CN Tower, **10.01 s** warm, 1,466 nodes at lift. `game._earth` was null throughout the Custom Location runs, which is what proves the hold came from the level test and not the old mode flag.
- **The cold hold is long and that is the trade.** A first visit to a city fetches DEM, imagery and an Overpass region: ~28-30 s behind the curtain instead of ~3 s onto bare ground. Warm it is ~10 s. `_LSS_EARTH_CURTAIN_MAX_MS` (45 s) is the ceiling.

#### ⭐⭐⭐ (v46.89) ASSAULT ON A CUSTOM LOCATION — and the async-build spawn latch it exposed
**Jump:** the `isAssaultMap || isGmapsSlot` line in `_visibleMapKeys` · the `_asltGmaps` branch in `_lssGmapsBuildLevel` (where `game.corridorPoints` is filled) · `_spawnPickSet` / `_spawnPickTake`.
Owner: *"we should put custom location into assault mode and cyberpunk city mode"*. Assault: done. Cyberpunk: **not done, see the entry below.**
- **A carousel entry was the small half.** The v34.68 note on that filter says why: assault maps' *"side-'A' defender end + champion room are load-bearing"*, and a gmaps level ships `rooms: []`. Three things had to be synthesised at build time:
  - a **champion room** — `spawnChampionField` looks for `sdfRoomData.find(r => r.champion === true)` and otherwise averages `game.levelSpheres`, which on a gmaps level is EMPTY, i.e. the objective lands at (0,0,0): the map origin at ground level, inside a downtown building;
  - a **defender end and an attacker end** — `_assaultSpawnSide(team)` returns the fixed side tag and `getValidSpawnPoint` filters `corridorPoints` on `p.team`; untagged points fall through to its x<0 / x>0 split, which on a streamed world is a meaningless axis;
  - both ends **clear of the city**, which the level can already answer.
- **⚠⚠ SPLIT BY BEARING, NOT BY RADIUS.** `tiles.spawnPoints()` lays its points out as CONCENTRIC RINGS around the drop origin — measured on Toronto: 64 points over 11,889 × 11,946 units on radii ~2843 / 4422 / 6002. The first cut put the field at their centroid and took the nearest and furthest thirds, which is the inner ring and the outer ring: two shells sharing one centre, **33 units apart**. Projecting every point onto the origin→field axis and taking the extremes of each half gives **7,726 apart**, defenders 514 from the objective and attackers 8,237.
- **⚠ The field is one of the level's OWN spawn points, never an invented coordinate** — those are already validated clear of the buildings, which is the map's standing rule for arena rooms (retag a verified centre). The anchor is the point at the MEDIAN radius, so the objective sits in the city rather than on the drop pin or out at the rim.
- **⭐⭐⭐ AND IT EXPOSED A BUG WIDER THAN ASSAULT: `buildRoomGraphLevel` IS ASYNC FOR A GMAPS LEVEL AND NOBODY AWAITS IT.** `_finishCommit`'s fresh-match path runs `buildRoomGraphLevel(level); ... player.position.copy(_spawnPickSet(getValidSpawnPoint(teamCode)))` — so on any Custom Location that pick is drawn against the PREVIOUS map's `corridorPoints` (or an empty pool, i.e. the 0,0,0 fallback) **and then LATCHED** into `_spawnPick`, which the first respawn of the round consumes ahead of a fresh draw (the v45.04 mechanism). **Measured in assault over Toronto: the player sat at (-40, 311, -393) — 3,939 u from the defender end and 3,852 from the attacker end, i.e. neither — and dying put them back in exactly the same spot.** Both gmaps arms now re-pick (assault) or re-latch (everything else) once the pool is real. Behind the curtain, so never a visible teleport.
- **Verified live:** assault carousel = `gmaps_user` + `assault_shifting` + `assault_causeway`; Toronto drop → `field -4401 818 433 | defenders 19 attackers 19 | ends 7726 apart`, `player dropped on side A -2520 829 1359` (1,640 from the defender end, 6,527 from the attacker end, 2,096 from the objective), champion field alive, 30 entities. **Regression:** The Causeway unchanged — 9 rooms, champion room present, field alive, 5 bots, zero console errors.

#### ⚠ (v46.89) CYBERPUNK CITY ON A CUSTOM LOCATION — NOT DONE, and what it needs
The other half of the same ask. Assault only needed a room skeleton; **cyberpunk city is structurally bound to the overworld hub in three places**, and each is a real piece of work:
- `_cyberSpawnField` **returns null without `HUB_CITY`** and places the objective at `HUB_CITY.x/z` inside `HUB_CITY.genome.radius`, validating clearance with `_hubCityRayHit`. On Earth that would have to become the drop city's centre, with the world's own `heightAt` / `raycast` doing the clearance test (`tiles.spawnPoints()` is the cheaper answer — it is what the assault branch uses).
- `_cyberApplyStart` → `_carrierSpawn({ sector })` → `_carrierAttach` builds the carrier's route from **`_carrierPortal(sector)` → `_carrierCityPoint()`**, stopping at `HUB_CITY.genome.radius + CARRIER.edge`. Sectors are a hub concept; Earth has no portals and no sector ring.
- `_cyberDeckPlace` stands the attacking fleet on that carrier's deck, so it inherits whatever the route decides.
`startCyberpunkCity` itself is easy (it is `startFreeFlight('cyberpunk')`, and the freeflight branch of `_visibleMapKeys` would just need `gmaps_user` beside the `hub_` maps) — the mode does not work over Earth until the field and the carrier route are re-sited. Left out deliberately rather than shipped half-built.

#### ⭐⭐⭐ (v46.91) "I SPAWNED KINDA MIDDLE OF THE ARENA" — two ways a spawn outlives its level
**Jump:** `_gmSeq` / `_gmStale` in `_lssGmapsBuildLevel` · the `_spawnPickSet(null)` beside the `game._buildSeq` bump in `buildRoomGraphLevel` · `_spawnPickTake` in `respawnPlayer`.
Owner: *"i went from elimination custom location, then after a match switched to shifting deep in the same elimination lobby... but i didn't spawn with my team, i spawned kinda middle of the arena."*
- **(1) THE GMAPS BUILD COULD LAND ON THE NEXT LEVEL.** `_lssGmapsBuildLevel` is async and awaits up to 25 s on `tiles.ready`; its tail then writes `game.corridorPoints`, `game.sdfRoomData`, `player.position` and the `_spawnPick` latch. Nothing stopped an in-flight EARTH build from writing all of that **onto whatever level was built next**. The new map's pool is replaced by Earth points, which in elimination carry **no `team` tag at all** — so `getValidSpawnPoint('A')` finds nothing, falls through to its `p.x < 0` half-split, and returns a coordinate kilometres out that the arena bound clamps toward the middle. That is the report, mechanism for mechanism.
  - `game._buildSeq` is bumped by `buildRoomGraphLevel` for every level, and the overlay IIFE in that same function has guarded on it since v37.00 — **this path simply never did.** Captured after the bump (buildRoomGraphLevel calls us), checked after the await, bail before touching any world state. Nothing leaks: the tiles are already parented and `_lssGmaps.cleanup` is what the next build calls.
  - **Verified:** bumping the sequence mid-build logs `[lss-gmaps] build is stale (seq 1 -> 2) - abandoning spawn placement for Custom Location`; the stand-in new-map pool survived intact (2 team-tagged points, not 64 untagged Earth ones) and the player was not moved.
- **(2) ⭐⭐ A SPAWN PICK OUTLIVED ITS MAP.** `_spawnPick` is a latch `respawnPlayer` consumes **ahead of** drawing a fresh point (v45.04), and **nothing in the file ever cleared it** — only set, or taken. A pick recorded on one map and never spent there (you did not die) stayed loaded, and the first respawn on the NEXT map used a coordinate from the old one. Harmless between two arenas of similar size; not harmless coming off a Custom Location. Now cleared once per level build, beside the `_buildSeq` bump, before anything fills the new pool — every legitimate writer runs after that line.
- **⚠ Not reproduced end-to-end.** The owner's exact sequence needs a full Custom Location match played to its end; these are the two holes in the code that produce that symptom, both proven to exist and both closed. Baseline after the change: Shifting Deep from a clean launch puts the player **186 u from their own `spawn_a`** and 2,339 from `spawn_b`, pool 184 points (11 A / 11 B).

#### ⭐⭐ (v46.92) THE RIFT-GUARD HITCH — 175 cold program links in one frame
**Jump:** `const _HZ_GUARDS` (gained `GAP`) · `function _hzGuardsBuildStep` · the `_guardBuild` branch at the top of `_hzGuardsFrame`.
Owner, with `?pbhud`: *"i found a hitch when the rift guardians came out to fight in exhibition"*, then *"i just got a hitch in chrome, but not the preview window"*.
- **The F8 mark says it outright.** `freeflight:hub_overworld`, build 46.91: a **1,124.9 ms** frame, `made` = **175 programs / 136 geometries / 29 textures created in that ONE frame**, `cold` = **175 links**, `getProgramParameter` (the blocking LINK_STATUS query) caught at 21 / 34 / 40 ms. The named sections account for only ~200 ms of it (`hub:ripple` 100, `hub:weather` 50, `hub:stream` 29, `hub:clip` 22) — **the rest is not in a section at all**, because linking happens at draw time inside `renderFrame`.
- **175 is not a mystery number.** The flight is `N: 4` hulls, `buildModelShipMesh` gives each 14 meshes + 14 materials (v38.92), and each material forks a program per render target / light count / depth pass: 4 × 14 × ~3. `_hzGuardsFrame` built **all four inside one `.then`**, so every one of those programs was created in one tick and first-drawn in the next.
- **⭐ CHROME-BUT-NOT-THE-PREVIEW IS THE CONFIRMING DETAIL, not a contradiction.** A preview that has been running all session has a warm program cache and re-links nothing; a fresh browser links all 175. Re-measured here after the fix: `renderer.info.programs` did not move at all while 24 guards spawned — which is exactly why the spike cannot be reproduced in a warm session and why the owner only saw it in Chrome.
- **The cure is the one this file uses everywhere else it hands the GPU a lot at once** (the earth publish budget, `_leSlicer`, the hub streamer): give it one at a time. The `.then` now only RECORDS the job (`p._guardBuild`) and `_hzGuardsBuildStep` builds **one hull per `GAP` seconds** (0.14) off the existing per-frame call — no timer of its own, dies with the portal like everything else there. `_lssPrimeTick` already runs 2 ms a frame from `gameLoop` and drains links that finished in parallel, so each hull's programs are linked and primed before the next arrives. `GAP: 0` restores the v35.15 all-at-once build.
- **⚠ Every stand-down condition is re-checked on EVERY step**, not just once in the `.then`: a flight now spans several hundred ms, so the world can move on part-way through (cavern entered, room went networked, round ended, portals rebuilt). Whatever is already flying is kept — it owns itself once it is in `game.entities`.
...
#### ⭐⭐⭐ (v46.97) "YOUR SHIP ACTUALLY SLIDES FROM POINT A TO POINT B" — the cinematic lineup
**Jump:** `lineCenterX` / `lineY` / `lineCenterZ` in `_lssStartSpectatorCinematic`.
- **MEASURED: `[cinematic] lineup on spawn | slide was 150 u`** on The Nexus (elimination, 3 ships). The log is permanent — it prints the distance the ship USED to travel, and it is the only way to see this once it is zero. Expect `slide 0`; anything else means the spawn moved after the lineup was sited.
- **⚠ NOTHING WAS WRONG WITH THE BLEND, and I checked before changing it.** The arrival blend flies our own ship from its lineup slot to `player.position` over the last 1.4 s ON PURPOSE (v36.76: *"our own ship's lineup mesh flies home to its restored pose on the same ease, so the dive visibly lands ON our ship"*). It is also a TRUE parametric lerp, not an accumulating one — every cinematic frame re-pins each mesh to its slot (`s.mesh.position.set(s.pos...)`) before the lerp, so camera and ship really do share the ease. My first theory (mesh accumulates, camera resets, so the ship converges faster) was WRONG and the re-pin is why.
- **THE TWO LOCATIONS WERE THE BUG.** The slot came from the spawn ROOM'S CENTRE (`spawnRoom.x/y/z` + a 10%-radius nudge, `cy + 30`) while the ship's real spawn is `getValidSpawnPoint` → a scattered `corridorPoints` entry in that room. So the blend always had a room-radius of travel to cover and faithfully flew it. Fix = centre the lineup ON the player's spawn; the lerp then has nothing to move.
- **The player is slot 0** (`sideIdx = 0` → `off = 0`), so the slot IS the line centre — which is why three assignments are the whole fix: `orbitCX/CZ`, `camPos`, `camTarget` and `camHeight` all derive from them, so the framing follows the lineup instead of needing to be re-aimed.
- **⚠ Only when the player is IN the lineup** (`myShips[0].isPlayer`, after `_moveFront`) — a spectator / bot-only flight has no ship to centre on and keeps the room centre.
- **⚠ Accepted trade:** teammates now spread about the player's spawn rather than the room's middle, so a wide line off to one side of a small room can clip a far teammate into a wall for the flight. Cosmetic, and the better trade — the lineup exists to be looked at, every ship is restored to its real spawn at the cut, and nothing collides during warmup.

#### ⭐⭐ (v46.96) "IT PUTS ME AT THE ORIGINAL SPAWN FOR A SECOND, THEN AT THE PORTAL" — and the cinematic slide
**Jump:** the `_HZ_CAVERN._returnPos` block in `_hzPortalsFrame` · `_s0.mesh.position.lerp(player.position, b)` in the spectator-cinematic blend-out.
- **The rift return applied its position BELOW the `game.state !== 'playing'` gate.** So the sequence was: the return rebuilds the hub → `respawnPlayer` drops you at the generic hub spawn → roundEnd → warmup → playing → and only THEN does the ring position land. The cover is long gone by `playing`, so the whole detour is on screen. It is the v45.x *"nobody should ever see themselves be moved"* rule, missed on this one path. Moved ABOVE the gate — the terrain checks just above it are the real precondition (a hub coordinate means nothing until the hub exists) and they are satisfied while the swap cover is still up.
- **⚠ AND IT LATCHES.** `respawnPlayer` consumes `_spawnPick` ahead of drawing a fresh point (v45.04), so without `_spawnPickSet` here the round-start respawn would move you a SECOND time right after this put you back. Set HERE, after the rebuild, because v46.91 clears that latch on every level build.
- **⚠ VERIFICATION WAS INCONCLUSIVE and the fix should be re-checked by flying it.** Entering through `window.__cavern.enter(0)` makes `cv.returnPos` the *current* position, so on the debug path the "wrong" and "right" spots are metres apart and the double-move cannot be told from ordinary flight. Measured round trip did return to the entry point, but a 735-unit move was still sampled during `playing` that I could not attribute.
- **⭐ THE CINEMATIC SLIDE IS THE SAME FAMILY, one line:** the blend-out runs `_s0.mesh.position.lerp(player.position, b)`, i.e. the lineup mesh is *interpolated* toward `player.position` across the blend. If the spawn is applied while that blend runs, the ship visibly TRAVELS between the two poses — the owner's *"your ship actually slides from point a to point b"*. v39.95 already fixed the SNAP half of this block ("rewriting it to the start-of-cinematic pose is the visible snap"); the slide is what remains, and it only shows when the lineup pose and the spawn pose genuinely differ.
- **⭐⭐ THE SHARED ROOT, worth fixing once rather than per-symptom:** *when is the player's spawn final?* Both reports are "something placed the player, then something else placed them again, and the second one was visible". The same shape has now been found in four places this session — the gmaps level (v46.89), the stale-build latch (v46.91), this rift return, and the cinematic. A single invariant — the spawn is decided once, before anything can draw the ship, and every later writer goes through the `_spawnPick` latch — would close the class instead of the instances. NOT attempted.
...
- **⭐⭐ (v45.42) THE BOT COMBAT BRAIN — cores, threat awareness, charges, locks.** Owner: *"i don't think the bots really know how to use their abilities... or they aren't setup so they can use them."* **Jump:** `_threatTick` · `_activateCore` / `_coreTick` · `_botSalvoStart` / `_botSalvoTick` · `_addLock` / `_lockedTargets` / `_consumeLock` / `_botLockTick` · `_railWind` / `_railTick` · `_chargeShotTick` · `_enemiesWithin` · `_creditAbilityCore` · the `_surgeT` clamp arm beside `_dashCap`.
  - **THE CORE IS THE SHIP'S OWN CORE.** `_useCoreSurge` is gone: one function, five 700-dmg rockets + a shield top-up, **identical for all seven ships** — a weapon no ship in the game carries. `_activateCore` dispatches on `loadout.core.name` to the real seven, built from the player's own numbers/flags (bot 0.6 handicap on direct damage). **Measured, live:** Mega Laser 7204 asked over 4 s (= 12000×0.6) · Mega Flame Chain 7083 across 3 targets with falloff · Mega Stun Bolt 11232 over 1021 aura ticks + 8 flurry bolts at 360 · Mega Barrage 15 rockets at 280 + `_surgeT` 5 s · Mega Tracker Rockets **26 projectiles, all 26 homing**, 250 dmg · AI Assist and AI Nanobots fire as buffs. All seven seen in live rounds; zero console errors over 35 s of core-heavy play; frame p50 6.9 / p95 8.7 / worst 10.5 ms (145 fps).
  - **THE METER CHARGES LIKE A HUMAN'S.** `coreMeter += dt * 1.5` (a wall clock, full in 66 s whether the bot fought or not) is **removed**. The player has no passive charge at all, so the bot's three sources are now the player's three: damage dealt ÷100 (`fireAtShip` + `_dealAbilityDamage`), +15 a kill (`Bot.die` **and** `playerDie` — a bot that kills YOU earns its core), +2 a cooldown-ability spend (`_creditAbilityCore`, and **not** for a free hold-shield tap, exactly like `executeAbility`). Source-audited: those four are the only increments left in the class. Verified +10 for 1000 dealt.
  - **⚠ `_dealAbilityDamage` measures the PLAYER victim by HP+shield delta** — `playerTakeDamage` returns nothing (all four exits are bare `return`s), so there is no dealt value to credit otherwise.
  - **UNDER FIRE, NOT HURT.** `_tryUseDefensive` gated on a HEALTH threshold, which answers a different question far too late. `_underFireT` is set by `Bot.takeDamage` (the only signal hitscan gives) **and** by `_threatTick`'s 8 Hz incoming-projectile scan (closing + would pass within ~260 u, lead ≤ 1.6 s) — the scan is the half that lets a bot dodge rather than react. **Measured: 36 defensive uses across 5 classes in 30 s, 29 of them at ≥90% HP.**
  - **⚠ THE HOLD SHIELDS WERE NEVER COOLDOWN ABILITIES.** Vortex Shield / Absorption / Fire Shield / Body Shield carry `cooldown: 0` (a human HOLDS them), so `(ability.cooldown || 14) * 1.4-2.0` charged them 20-28 s — four ships whose defensive identity is a held shield could raise it twice a round. They now re-arm on a 6 s duty cycle; real cooldowns went 1.4-2.0× → **1.0-1.25×** in all three slots (the inflation was the single biggest reason bots looked ability-less).
  - **EVASION IS MOVEMENT, NEVER A WARP** (owner was explicit). `_threatTick` commits a ~0.5 s juke **perpendicular** to the incoming axis (flying away from a beam keeps you in it), re-rolled with a per-id side bias so wingmen peel apart; blended at 0.85 into `moveDir` and the turn lerp goes dt*2 → dt*5 while dodging. **Only on the plain combat path** — `_raceWaypoint`/`_arenaVia` exist because a straight line is not flyable, so jinking there flies into the rock the router was avoiding. **Measured: motion perpendicular to the shooter's line 0.322 → 0.701 of speed.** SLAYER's **Teleport is refused outright while `_underFireT > 0`** (0.4 s back-off) and unchanged otherwise — measured 0 u under fire, 776 u calm.
  - **⚠ A SPEED SURGE HAS TO RAISE THE CAP.** v43.18 half-fixed this (taught the clamp `dashSpeed` when `dashActive`) and noted in passing that PUNCTURE's bot **Afterburner is inert** — it adds `flightSpeed*1.6` into velocity and the next frame's clamp rescales it away. `_surgeT`/`_surgeSpd` beside `_dashCap` is the general fix; Afterburner and Mega Barrage both use it.
  - **PUNCTURE CHARGES AND PARTIAL-CHARGES** (owner). `_railWind` runs from inside the fire gate — which has already proved range, facing and LOS, i.e. the bot only winds while it genuinely has the shot. Human's numbers: 2.5 s to full, `1 + charge*3` (tap = base 1000, full = 4000), spent by the shot hit or miss, 0.2/s bleed-off (`_railTick`). `_railWant` (0.35..1.0, re-rolled per shot) is what makes it read as a pilot. **Two partial releases: point blank (<500 u) and under fire (≥0.25).** Measured live: `chg 0.809 → mul 3.43` at 886 u, `chg 0.217 → mul 1.65` under fire at 500 u, `chg 0.003` snap shots at 153/438 u. The shot also draws through `_spawnRailgunSpiral` now, not a generic tracer.
  - **⭐ TRACKER LOCKS EXIST BOT-vs-BOT.** Every earlier cut said "bot-vs-bot has no lock bookkeeping and homes outright" — which is exactly why the ability needed a second, invented rule. `_botLocks` is the missing ladder (built in `fireAtShip`, aged 1 mark / 5 s like the player's decay), so **one** rule now serves bot→player, bot→bot and bot→monster: the human's. No full lock = **nothing fires**, partial locks preserved (0.6 s back-off so an unlocked TRACKER stops re-raycasting 4×/s); a full lock fires **5 homing rockets per locked ship at 1000 dmg / 80 splash / 900 speed** and consumes it. The old version fired 3 at 1100/100/700 that flew **straight** unless the player happened to be at 3/3 — that is the "tracker rockets don't seek" report. Measured: no lock → 0 projectiles; player lock → 5/5 homing on PLAYER, lock consumed, cd 0.27; bot lock → homing on that bot, lock consumed. The v37.05/v45.39/v45.40 `_rkHome` / `_baseCd = 7` machinery is retired with it.
  - **OTHER ABILITIES BROUGHT TO THE HUMAN'S NUMBERS:** Cluster Missile 800/**250** + `clusterDmg 500`/`clusterDuration 5`/`sizeMult 3` (the card promises "Impact 800 + 500 DPS for 5s" and `cluster = true` alone never said how hard or how long) · Stun Bolt **two half-damage bolts out of the two muzzle markers totalling 2000** (was one 1200 bolt from the hull centre) · Rocket Salvo **5** rockets at 700/100/800 (was 3) · **Charge Shot now spools 1 s with a growing glow** before it fires (it was instant — a 3200-dmg shot with no tell at all).
  - **⚠ `_botSalvoTick` is the bot's own staggered salvo** — `_enqueueStaggeredRocketSalvo`/`_drainStaggeredRocketSalvo` are written entirely against the `player` singleton (camera-relative aim, screen-fraction launchers). One rocket per drain, out of the hull's `gunN` markers, homing target re-validated each release.
  - **⚠ All five per-frame ticks (`_coreTick`, `_botSalvoTick`, `_chargeShotTick`, `_botLockTick`, `_railTick`) run at the very END of `update()`** so they can use `_tempVec3a/b/c` freely; `_threatTick` runs just before the steering block because it borrows `_tempVec3d` for one cross-product. A stunned or warmup bot returns before all of them, which is intended.
  - **⚠ The ability gate is now `(_ctAlive || this._underFireT > 0)`** — a bot shot from behind by something it has not acquired still has to be allowed to shield.
...
#### ⭐⭐ (v46.90) TELEPORT OUT OF A TETHER · and the SYPHON CORE's Inner Spark wash
**Jump:** the `'Teleport'` arm of `executeAbility` (`_tpRooted`) · `function _innerSparkRipple` / `_innerSparkTick` / `_ISR_CORE` / `_isrCoreOpts` · the `'AI Nanobots'` arm of `activateCore` · the `inner_spark` peer handler.
- **⭐⭐⭐ "slayer should be able to teleport out of a tether" — the break-out was there since v35.63, the HOP was not.** That code clears `game.playerRootTimer` and re-arms the trap, so the ability always *looked* like it worked. But the blink itself is gated on **`speed > 5`**, and a tether zeroes your velocity every frame — at the trap (`eff.rootTarget.velocity.set(0,0,0)`) and in `updatePlayerMovement`, which refuses to move you at all while rooted. So a tethered Slayer always measured speed 0, fell into the STATIONARY arm and "phased in place": **root cleared, ship not moved** — and the trap is deliberately re-armed rather than consumed, so it caught them again on the spot. The escape ability could not escape the one thing it exists to escape.
  - `_tpRooted` is captured **before** the v35.63 clear erases the evidence (it reads `playerRootTimer` *or* any tether whose `rootTarget` is the player), the gate becomes `speed > 5 || _tpRooted`, and the heading comes from **`forward`** instead of the zeroed velocity. Everything downstream — the ground clamp, the three retries, the zero-distance refund, the stun sweep — is untouched and now applies to the escape too.
  - **Verified, four cases:** moving + not rooted **400** (unchanged); stationary + not rooted **0**, still phases in place (the v44 behaviour is deliberately preserved — only a ROOT converts a standstill into a hop); stationary because `playerRootTimer` **400** with the root cleared; rooted by a real `tether` worldEffect **400**, with `triggered`/`rootTarget` reset so the trap is re-armed, not consumed.
- **⭐ "syphon's core should do similar animation as inner spark, just brighter and a bit longer".** `_innerSparkRipple(owner, opts)` now takes `{ dur, gain }`; defaults reproduce the v44.17 ability exactly so every existing caller is untouched, and the core passes `_isrCoreOpts()`. The peer event carries **`core: 1`, a flag rather than the numbers**, so the two stay tuned in one place.
  - **⚠ GAIN IS BOUNDED BY THE CLIP, not by taste.** Opacity is `min(1, 0.9 * gain * sin(pi*k))`, so any gain above ~1.11 SATURATES across a band in the middle and the wash goes flat-topped — the same trap as the fire shader (a brightness multiply into a clamped target is not brighter, it is flatter). **Measured: gain 1.6 clipped 51% of the run**; 1.3 clips 30%; **1.25 clips ~26%**, which is a short hot top with the sine's shape intact. The part of `gain` that does NOT clip — the travelling hit-flash radius, `hull * 0.55 * gain` — carries the rest of the read.
  - Shipped `_ISR_CORE = { dur: 1.7, gain: 1.25 }`, live knob **`window.__syphonCoreFx = { dur, gain }`**. **Measured A/B on the hull:** ability peak **0.90**, gone at **0.98 s**; core peak **1.00** with ~28% of samples at the clip, gone at **1.70 s**. The flash cadence scales with `dur` so a longer wash is a longer discharge, not the same few pulses stretched thin.
  - Re-triggering replaces rather than stacks (the v44.17 one-per-ship rule), so the core firing while the ability's wash is up simply restarts it bigger.
- **⚠ HARNESS, and it cost real time:** with the Browser pane HIDDEN, rAF freezes → the game loop stalls → the launch curtain never lifts → `#ship-select` stays `.active` → **`_abilitiesBlockedByUi` (v46.79) refuses every ability**, so a synthesised keypress does nothing and reads exactly like a broken fix. Clear `.active`/`.lss-launching` and the overlay by hand to test ability logic without frames — and note a keydown with no matching **keyup** leaves `input.keys[k]` stuck true, so the next press has no rising edge and never fires.
...
#### ⭐⭐ (v46.86) THE BOT VORTEX'S BEAMS — out of gun1/gun2, and down the nose
**Jump:** the `Mega Laser` arm of `_coreTick` · the `'Laser'` arm of `_tryUseOffensive` · the `_coreFace` block above `this.targetDir.lerp(...)` · `function spawnTracer` / `function _spawnSingleTracer` (both gained a trailing arg) · `function _vxTracerRetract`.
Owner: *"there's a bug with the bot vortex using the mega laser core... it looks like it disconnects from the ship, but it should be connected the same way the other guns are through the glb muzzle G1, and G2"*, then *"it looked like the vortex ship wasn't pointed at the enemy when using mega laser... like it was coming out the side of his weapon"*. **Two independent defects, and fixing the first is what made the second visible.**
- **(1) THE ORIGIN WAS THE HULL CENTRE.** Both tracers left `this.position`. **MEASURED on the Vortex hull: 85 u from either gun marker on a `hullLength` of 100** — most of the ship. Every other bot muzzle had been moved to the markers years ago (`shipMuzzleWorld`, v32.21; the Stun Bolt got the same treatment in v45.42); these two beams were missed because they are the only bot weapons that draw a TRACER instead of spawning a `Projectile`.
- **(2) THE RETRACT IS FOR BOLTS, NOT BEAMS.** `_vxTracerRetract` peels a streak's near end forward after 30% of its life — right for a shot in flight, fatal for a beam. At the core's 20 Hz respawn the shortest layer (the 0.10 s hot core) starts peeling 30 ms in, so **for 20 ms of every 50 ms every live layer was mid-retract**. MEASURED worst drawn near-end: **727 u from the nearest gun**. New `beamPin` flag (5th/6th arg on `spawnTracer`, 7th on `_spawnSingleTracer`) holds the near end on its anchor and skips the peel.
- **⚠ THE HAZE TAILS ONLY HALF-FOLLOWED THE ANCHOR.** v38.68 taught the `tracer` arm of `updateEffects` to re-read the live marker each frame but never the `tracerTail` arm — so the two widest layers of the five (radius 7 and 9) stayed at the spawn point. **Gated on `_beamPin`, NOT on `_anchor`:** an anchored BOLT's tail is meant to be left behind, and the anchor list includes the dash speed lines off the engine orbs, where trailing IS the effect.
- **⭐⭐ (3) CHANNELLING A BEAM MEANS POINTING AT WHAT YOU ARE BURNING.** The main guns have always been nose-locked (the fire gate is `dot > 0.7`, ~45°), but `_coreTick` fired along `(target - position)` with no facing rule and **nothing in the steering knew a core was running** — so a Vortex mid-dogfight-break burned a four-second beam out of its own flank. The heading now IS the aim for the core's duration, at the dodge block's hard turn rate. Folded into the SINGLE existing `targetDir.lerp` write and kept under the same `!_raceWaypoint && !_arenaVia` guard — two writers of one heading is how it starts fighting itself. Its own scratch vector, because `moveDir` is already one of `_tempVec3a/b/c` depending on which branch set it.
- **The `'Laser'` ABILITY had all of the same defects** two branches of the AI over — same ship, same guns, same hull-centre origin. It now fires one pair per gun (the human's Laser draws an arm from EACH gun, `_vortexGunPair`) and takes the main weapon's own `dot > 0.7` nose gate. Refused with a plain `return`, matching `_tryUseOffensive`'s range and LOS gates, so the cooldown stays at zero and the bot fires the moment it comes round instead of being sent away for half a second. **No `beamPin`** — a one-shot bolt should retract.
- **⚠ A SCRATCH VECTOR CANNOT BE THE FAR END.** `_spawnSingleTracer` keeps `toVec` **by reference** as `_retractTo` and re-reads it every frame, so the old `this._tempVec3c` beam end meant every live streak's far end jumped to whatever the class wrote next. Both sites now allocate a real `Vector3` per tick.
- **⚠ COSMETIC ONLY, deliberately.** The LOS raycast and `_dealAbilityDamage` still run off `this.position` — the v32.21 rule for every muzzle move in this file.
- **Verified live** (free flight, `__dbg.spawnBot('VORTEX')` + a held `_coreName`): drawn near-end **727 u → 0.00 u** worst case, stationary AND at 195-320 u/s with the bot flying its own AI; both `tracer` and `tracerTail` layers pinned; both barrels firing (`perGun` 10/16 core, 10/10 ability); heading-to-target dot **0.9999**. Regressions: with the core off **zero** `_beamPin` streaks exist anywhere, the player's own shots are still anchored to their own markers and unpinned, other bots' shots are unanchored exactly as before. Frame rate A/B in the same scene, beam on vs off: **identical `30 fps 1870x1111`**. Zero console errors.
  - **⚠ Harness note:** the pane throttles rAF when the tab is hidden, so an in-page fps counter reads ~1 fps and means nothing — read the game's own on-screen readout off a screenshot instead. And `Object3D.lookAt` aims local **+Z**: probing a hull's facing with local -Z reports 179° off when it is dead on target (these hulls are nose-along-X in model space, see the Blender pipeline notes).

#### Ship-select / loadout / perk / difficulty UI — `~L49314`
**Jump:** `function buildShipSelect` (~L49316)
- **⭐⭐ (v46.85) CONFIRM LOCKS YOUR SHIP AND YOUR PILOT PERK.** Owner: *"after i hit confirm, i was still able to change my ship and ability, that shouldn't happen. confirm should lock those"*. **Jump:** `function _lssPickLocked` / `function _lssSyncPickLockUI` (right above `_lssRefreshLaunchRow`) · the guard at the top of `previewLoadout` · the guard in `_setStoredPerkId` · the `_lssPickLocked()` fork in `buildShipSelect`'s landing key · `#ship-select.ss-pick-locked` CSS.
  - **It was wrong in two concrete ways, not just cosmetically.** (a) The room ALREADY shows your pick the moment you tick CONFIRM — v45.17 puts the hull on the `ready` event so every fleet card can paint it — so browsing on afterwards made everyone else's launch decision a description of a ship you had walked away from. (b) `previewLoadout` writes **`game._ssKey`, which is exactly what a peer's LAUNCH commits on your behalf** (the `launch_now` handler), so a confirmed player who kept clicking chips launched in whatever was last on the stage.
  - **TWO GUARDS, BOTH AT A FUNNEL.** `previewLoadout` is the one function every ship-picking input reaches (chip listener, ‹ › arrows, gamepad D-pad, XR panel row, VR stick — the same reason v36.36 stages the between-rounds pick there); `_setStoredPerkId` is the single write point for the perk (the cards, `_cyclePerk` for LB/RB, and the XR perk arrows via `_cyclePerk`). Gating at those two covers every path and makes a future sixth one safe.
  - **⚠ `previewLoadout`'s guard compares against `_previewedKey`, NOT against the key alone.** Several callers are REPAINTS, not picks — the WebXR-support probe refreshing an open picker, and `buildShipSelect`'s landing call — and they ask for the ship already on the stage. Refusing those would leave a re-entry with an empty stage and no confirm row. A first paint (`_previewedKey` null) is likewise always allowed, and **`buildShipSelect` now lands on `_previewedKey` when locked** (it used to ask for `player.loadoutKey`, i.e. last match's hull, which the guard would have refused).
  - **⚠ NOT `_lssRoomSettingsFrozen()`.** That predicate also freezes for a late joiner (`net._dropin`) and **v44.92 is explicit that a late joiner still picks their own ship and perk** — only the three room-wide controls are off limits. `_lssPickLocked` asks about CONFIRM and nothing else.
  - **⚠ `game.state === 'select'` IS LOAD-BEARING** in the predicate. Between rounds previewing IS picking (v36.36) and there is no CONFIRM in that flow — v45.13 records that `_ssConfirmed` is left SET from round 1 — so asking `_ssConfirmed` alone would weld the swap window shut.
  - **The affordance follows the `.ss-map-locked` convention:** `#ship-select.ss-pick-locked` makes the chips and the perk cards `pointer-events:none`, dims the **un**selected ones to 0.38 (the selected one keeps full strength — "what am I locked into" is the one thing those rails still have to say), and greys + `disabled`s `#ship-car-prev/next`. `#perk-prev`/`#perk-next` stay live: they only SCROLL the track. CONFIRM's existing tooltip already read *"click to unlock and change ship, map, mode or speed"*.
  - **Verified live (1920×1080 and 1440×900, real mouse clicks):** unlocked, a chip click / `__roundSwapProbe({pick})` / a perk card click / `#ship-car-next` all change the pick. CONFIRM → `ss-pick-locked` on, both arrows `disabled`, `elementFromPoint` over a chip returns `#ship-carousel-track` and over a perk card returns `#perks-grid`; `__roundSwapProbe({pick:'SLAYER'})` and a synthetic `.click()` on an unselected perk card **both refused** (`_ssKey`, hero plate, `player.perkId` and `lss_perk_id` all unchanged). Un-tick → everything live again. Zero console errors.
- **⭐ (v46.85) THE FLEET COLUMN SPANS TOP TO BOTTOM, AND THE ART TAKES THE SLACK.** `#teammates-strip` was `max-height: calc(100vh - 122px)` (content-driven); it is now `bottom: 14px`. The v35.20 note said *"two short fleets shouldn't render as a tall mostly-empty panel"* — written when a half could hold one chip; **since v45.41 both fleets are always three seats**, so the content is six cards whatever the room looks like. `.fleet-half` / `.fleet-chips` went `flex: 0 0 auto` → `flex: 1 1 auto` (min-height left at its `auto` min-CONTENT default, so a full room overflows and `#fleet-scroll` scrolls rather than squashing), and `.fleet-chip` grows to a `max-height: 210px` cap with the growth going to the picture.
  - **⚠⚠ THE ART NEEDS A STATED `flex-basis` AND A STATED `min-height`, and it is the same trap twice.** The thumbnail is a 4:3 `<img>` at 100% of the card's width, so with `flex-basis: auto` its **intrinsic** height is the item's base size, and with `min-height: auto` that same intrinsic height is its **floor** (the automatic minimum size of a flex item is its min-content size). **MEASURED that way at 1920×1080: 180px of card width made every picture 135px tall, six cards came to 1104px inside a 998px column** — the image was sizing the card and the column simply overflowed. `flex: 1 1 64px; min-height: 64px` on `.chip-art` / `.chip-avatar-wrap` / `.chip-thumb-empty` takes the image out of the sum; the `<img>` then gets `height: 100%` and `object-fit: cover` crops it.
  - **⚠ The landscape-phone block must pin ALL of it back** (`flex: 0 0 auto !important; min-height: 0 !important`): the art is a fixed 42px there, and without the `min-height: 0` the `≤1200` floor of 54 wins and leaves 12px of empty plate under every hull (measured 54px art / 85px cards at 844×390, against the 74px that breakpoint means).
  - **MEASURED, six viewports, zero page scroll and zero panel overlaps beyond the pre-existing full-width `#ship-select-header` / `#ship-hero` bounding boxes:** 1920×1080 (column 44→1066, six 151px cards, 117px art, `scrollHeight === clientHeight`), 1440×900 (121px cards, no overflow), 1366×768 (cards at their 106 floor, 40px of scroll — **better than the 79px the old content-height column had**), 1150×700 and 1000×800 (168px column, 96/102px cards, rails from 198), 1600×600 and 1000×600 (`top:74`, scrolls), 844×390 (73px cards / 42px art — byte-for-byte the pre-change geometry), 880×700 and 760×900 (column hidden, rails full width).
...
## v41.00-41.12 — water displacement: the spray root, the sheet, and one undeclared variable

Owner: *"I don't really like the way the ship and weapons interact with the water... The way the water
displaces and splashes when objects move fast at angles, we need to fix. It would also drag with it
when it goes under water, and drag water droplets with it when it surfaces out of the water."*

### The physics, and where the old model was inverted

| what | source | what the code had |
| --- | --- | --- |
| spray-root speed `V·cot β` | Wagner (1932); the flat-plate impact measurements call it the *geometric intersection line velocity* | gated the whole splash on the VERTICAL speed alone |
| spray direction = free stream **reflected about the stagnation line** (swept-wing decomposition) | Savitsky & Morabito, *Origin and Characteristics of the Spray Patterns Generated by Planing Hulls*, JSPD 27(2):63-83 | an invented ±90° fan with a random forward lean |
| droplet speed **> 2×** the contact-line speed, rising with Fr | arXiv:1804.01068 | 0.16-0.60× — three to twelve times too slow |
| wake angle narrows as **U⁻¹** above Fr = U/√(gL) ≈ 0.5 | Rabaud & Moisy, *Ship Wakes: Kelvin or Mach Angle?*, PRL 110 214503 | a fixed offset with no angle in it at all |
| cavity elongates, **deep-seals at ~half its depth**, then a **Worthington jet** | Truscott/Epps/Belden, Annu. Rev. Fluid Mech. 46:355-378; Duclaux et al., JFM 591:1-19 | nothing |
| **~20° magic angle** minimises the speed to ricochet | Clanet, Hersen & Bocquet, *Nature* 427:29 | ships passed straight through |

⚠ **The old gate was not under-tuned at angle, it was inverted.** A ship crossing at 400 u/s on a 5°
path has Vn = 35, which scored **I = 0.036** — one droplet or none. Reality does the opposite: the
shallower the angle, the more violent the sheet, because as θ→0 the spray-root speed tends to
V/β_hull independent of the angle. Measured on the shipped model: **0.036 → 0.501** at 400 u/s / 5°,
and **0.099 → 0.729** at 900 u/s / 3°, with a vertical entry on a reference hull unchanged to 3 decimal
places (the spray root is normalised by the reference hull's tan β precisely so it cannot regress the
case that was already right).

### ⚠⚠ THE JELLO: every seed was ~20× past the display's own ceiling

Owner: *"something unrealistic about it... maybe looks too jello like when the boats and weapons hit it"*.

What you see is the displaced mesh, whose vertex shader is `disp = uRelief * tanh(h * uGain / uRelief)`
with uRelief 9 and uGain 95 — **96% saturated by h = 0.19**. Measured with `__waterProbe()` after a hard
entry on the pre-41.02 build:

| | peak h | saturated texels |
| --- | --- | --- |
| v41.01 and every build before it | **3.60** | **70.6%** |
| v41.03+ | 0.53 | **0.7%** |

Seven of ten disturbed texels were a flat-topped 9-unit plateau, so a pistol shot, a ship at cruise and
a ship slammed in at full speed all produced the *same* rounded dome. `impactPeak` was 50; it is 0.135,
measured so a hard entry lands at 8.2 of the 9 units the display can show. A seed is also part
**impulse** now (`uSeedImpulse`) rather than a pure pluck — a pluck sets a displacement AND a matching
velocity and then lets go, which is a plucked membrane, which boings.

⚠ **And the first fix was the wrong instrument.** v41.02 put the recalibration in a GLOBAL scale at the
seed funnel, which cut the wake 14× along with the impacts — owner: *"now the water barely moves when
the ship touches it"*. The wake family never multiplied by `impactPeak` and was never broken. The owner
had localised the fault in their first sentence (*"when the boats and weapons HIT it"*) and I fixed it
somewhere else. **When a report localises a fault, fix it AT that location; a global knob is not a
cheaper version of a targeted fix, it is a different and worse change.**

⚠ `uSeedImpulse` 0.65 was then too much of a good thing: an impulse must integrate up over ~a quarter
wave period before it is visible, so at 340 u/s the crest arrived 27 units behind the hull and kept
growing after it had gone — owner: *"something lags behind when you move through it... like it's
sticky"*. 0.25 keeps the crater-and-radiate character without the lag.

### ⚠⚠⚠ `_beta` WAS NEVER DECLARED — four builds of silent total failure

v41.04 added two references to `_beta` inside `_swSpawnSplashV` while the deadrise was still computed
inline under a different name. **There is no `_beta` anywhere in the file.** Every call that got past
the intensity gate threw a ReferenceError before spawning a single droplet: the planing sheet, the
impact crown and every weapon plume, from 41.04 to 41.07. That is the whole of the owner's *"there's no
splash"* and *"it's incomplete"*.

⚠ **And it took the rest of the frame with it.** The planing emitter calls it bare inside
`_swRippleTick`, whose only caller wrapped it in `try { ... } catch (_) {}` — so on every frame the ship
was skimming, the throw unwound past the wake trough seeds, the crossing impacts, the ricochet,
`R.prevAbove = above` and the whole of `_swWetTick`, and was swallowed in silence. **That is also why
the ricochet never fired in any test**: `prevAbove` was not being updated on exactly the frames that
mattered. I spent an hour on that as a separate mystery.

Why nothing caught it: `node --check` parses, it does not resolve scopes, so an undeclared identifier
is a clean syntax tree. The console was clean because the exception never reached it. And my own
verification measured particle COUNTS and tanh saturation — numbers that stayed plausible while the
thing they described was not running.

**What found it: `window.__waterFX()`, the emitter census (v41.07).** One counter on each of the eleven
emitters, asking not *"is this the right amount"* but *"did this happen at all"*. It read
`plane: 68` calls against `sheet: 0, crown: 0` in a single line.

| | before 41.08 | after |
| --- | --- | --- |
| sheet droplets | **0** | 250 |
| crown droplets | **0** | 169 |
| ricochets | **0** | 4 |
| Worthington jet | — | fires |

The ripple tick's catch now records to `window.__water.tickErr` / `.tickErrN` and logs once, and the
census surfaces it. **A `catch (_) {}` around a per-frame system is a place where a total failure can
hide indefinitely.**

### The sheet (v41.09-41.12)

Owner: *"don't do two sheets... make the one sheet bend and warp and fractalize"* — which is also the
more honest physics. A spray sheet is one continuous film leaving the whole spray root:

- **BEND** — the root is a curved line and the direction the film leaves rotates along it (ψ is
  interpolated across the span), so the sheet wraps from one chine, around the keel contact, to the
  other. Every earlier version picked a side with a coin flip, which is why it read as a blob stuck to
  the hull.
- **WARP** — one travelling wave along the root, amplitude growing with distance out (the film is stiff
  at the root and unstable where it has thinned). Not per-droplet noise.
- **FRACTALIZE** — film → ligaments → droplets → finer droplets (Squire / Rayleigh-Plateau), so a parcel
  draws a breakup LEVEL from a geometric distribution and its size, scatter, reach and life all follow.

⚠ The reach is also what unsticks the spray from the hull: a parcel is placed ALONG the sheet at the
distance its level puts it, so the shape exists the instant it spawns. **v41.09 overdid it** — 2.75
beam-widths and a 0.15 size floor spread the same count over ~10× the volume and it read as nothing.
41.10 pulls the reach to ~1 beam-width, floors the cascade at 0.27 and doubles the parcels per burst.

v41.11 adds the **rooster tail** (its own transom emitter — there was none; what looked like an arc was
just bow parcels being left behind) and **crest dots**, the underwater-bubble look in air: small, bright,
low-gravity specks that hang off the crests we already know the positions of, plus the crest-break
scan's own peaks. Rooster height was tuned against screenshots: 1.0 read as a low haze, 2.5 as a wall,
**2.0** is a clean column that still falls as an arc.

### Knobs (all on `window.__water`)

`oblique` `plane` `entrain` `shed` `wxCap` · `seedScale` `impactPeak` `seedImpulse` `crater` ·
`trim` `waveG` `waveFrc` `jet` `jetDelay` `skip` `skipAngle` `skipSpeed` ·
`sheetBend` `sheetWarp` `sheetFrac` · `rooster` `dots` · `breakPool`

`seedScale = 1` + `impactPeak = 50` restores the pre-41.02 surface exactly, in two assignments.

### Instruments added

- **`window.__waterFX([reset])`** — the emitter census plus the live gate state. Any zero after a flight
  that should have triggered it is a dead gate, not a tuning problem.
- **`window.__waterProbe()`** — reads the heightfield back and reports what the display's tanh does with
  it. `satPct` is the number that matters; anything much above a few percent means the surface has lost
  its dynamic range and will read as jello however the seeds are shaped.
- **`window.__obliqueTest(V, deg)`** — the spray root, ψ, which spray family, the sheet's world speed as
  a multiple of ship speed, Fr and the wake angle, for any approach.
- **`window.__groundY(x,z)` / `window.__waterAt(x,z)`** — ⚠ three of this session's measurements were
  taken over dry land before these existed. The hub lakes are small (the one used here is ~1200 units
  across); never assume the spawn or the origin is water.


## v41.13-41.28 — the crest, and weapons that hit the water like the ship does

### ⚠⚠⚠ THE PROBE WAS READING A MATERIAL THAT IS NOT ON SCREEN, AND I QUOTED IT FOR TEN BUILDS

`window.__pw` (relief 9 / gain 95) belongs to `_swBuildHubWaterOverlay`, which **returns `null` on its
second line** — the mesh is never built and `__pw` is never created by the game. Every *"the crest is
pinned at 9 units"* reading came from constants belonging to a material that does not render.

The water you actually see is the mesh built around **L29900**:

```glsl
_hx  = h * uGain / uDispScale ;   disp = uDispScale * (_hx / (1 + |_hx|))
```

a rational sigmoid with a **180-unit** ceiling, live-tuned by `window.__water.disp` / `.gain`. The probe
now reads the live uniform off `game._hubWaterDispMat` (v41.13), and the stale comment that was the
source of the wrong premise is gone (v41.15).

### ⚠⚠ THE CEILING WAS NEVER THE CONSTRAINT — THE SLOPE WAS

`uDispScale` appears in the numerator too, so raising it alone does almost nothing. **Measured live:**

| change | crest at the hull |
| --- | --- |
| `uDispScale` 180 → 900 | 51 → 67 units |
| `uGain` 180 → 650 | 51 → 150+ units |

For these amplitudes it reduces to `disp ≈ h · uGain`. The heightfield was responding to speed properly
all along (h = 0.17 at 80 u/s → 1.22 at 500 u/s, a 7× range); the display was flattening it.

### ⚠⚠ ...BUT A FLAT GAIN WAS THE WRONG SHAPE, AND IT WAS THE v41.02 MISTAKE AGAIN

At `uGain` 650 the transfer function's knee (`h = D/G`) lands at **0.277**, inside the working band —
the measured near-hull crest is 0.41. Local slope collapses **565 → 106 → 23** across h = 0.02..1.2, so
h 0.41 gives 107 units and h 1.20 gives only 146: tripling the field buys 36% more crest. That is the
same compression that made every impact look identical before v41.03. And being a slope, it lifted
ambient leftovers from 8.6 units to **27.5** — permanent chop on untouched water.

**A crest-only expander does what a gain cannot.** Shape the height *before* the clip:

```glsl
hs = h * (1 + uCrestQ*|h|)          // uCrestQ 11, uGain 100, uDispScale 180
```

so h→0 is governed by `uGain` alone (calm water unchanged) and crests turn quadratic and outrun the
clip. Measured: ambient 5.1 (was 27.5), crest 100 at 330 u/s, 144 at h 0.8, 161 at h 1.2.

### ⚠⚠ THE WAKE VANISHED OVER WATER BECAUSE THE NORMAL DESCRIBED THE OLD SURFACE

Owner: *"the wake disappears when it's seen over top of water... if it's in front of a mountain or land
it can be seen"*. `(hL-hR)*uGain` is the correct normal only while `disp = h*uGain`, which stopped being
true when the expander went in — the real crest slope is ~2.5× what it reported. A 139-unit crest was
being *shaded* as a gentle ripple: normal near vertical, fresnel and specular identical to the flat
water beside it, reflecting the same sky. **A shape change has to be carried into the normal.** They
now difference the shaped displacements over the taps' real 4-unit separation.

And it never whitecapped: foam keyed on height alone (`vDisp > 0.47` to clear `uPeakLo` 0.32) while the
crest runs 0.41–0.70. **A whitecap is a steepness event** — real water breaks near slope 1/7 — and
nothing in the shader looked at slope. `peak = max(peak, steepF * uSteepFoam)` keeps the old height path
intact and adds the term that separates a wake crest from ambient swell.

⚠ I sized the steepness threshold from the PRE-expander geometry and 0.085 whitened the whole lake.
Bisected against screenshots: everything foams at 0.085, nothing at 0.65, the crest sits near **0.50**,
and it needs a strength cap (`uSteepFoam` 0.55) because foam mixes to 90% opaque and full strength
paints a wall.

### ⚠ `vDisp` IS NOT THE DISPLACEMENT

The fragment keys foam and spray off it (`uPeakLo`, the eml weights), so tying it to the geometry means
every height change silently re-tunes the foam — under `uGain` 650 foam started at h ≈ 0.13, i.e. on
ambient chop. It is a fixed, shape-independent measure of disturbance (`h/(1+|h|)`), so the crest knobs
can move freely.

### The rooster tail, and why the knob is not the height

There was **no stern emitter at all** — what looked like an arc behind the boat was bow-contact parcels
being left behind. `_swRooster` launches at 45–75° off the transom. ⚠ The apex is **not** proportional
to the knob: the particle integrator's per-frame drag eats the launch speed inside ~0.13 s, so height
goes as roughly `k^0.7`. Sampled peak height over a run: **k=2 → 78 units, k=5 → 147, k=8 → 304,
k=12 → 450**. Owner asked for 4×; 8.0 is the 3.9×. Reading it off the knob would have been out by 2×
in either direction.

### One sheet: bend, warp, fractalize (v41.09-41.10)

Owner: *"don't do two sheets... make the one sheet bend and warp and fractalize"*. BEND — ψ interpolated
across the spray root so the film wraps chine-to-chine. WARP — one travelling wave along the root,
amplitude growing with distance out. FRACTALIZE — a geometric breakup level drives size, scatter, reach
and life. ⚠ v41.09 reached 2.75 beam-widths with a 0.15 size floor: same droplet count over ~10× the
volume, and it read as nothing. 41.10 pulls the reach to ~1 beam-width and floors the cascade at 0.27.

### Weapons (v41.19-41.28)

⚠ **Coverage was never the problem.** All weapons already reached the water in v40.99; the look was the
problem. Two real defects:

1. **The seed was positive** — since v33.21 every round made the surface *bulge up* at the point of
   impact. A projectile punches a cavity and throws the displaced water as a rim around it. Same
   inversion v41.02 fixed for the hull, never carried across.
2. **Nine call sites, every radius and amplitude a LITERAL** (65/0.08, 78/0.09, 90/0.14, 110/0.12,
   140/0.07). A Gatling round and a railgun slug punched identical holes.

**The fix that mattered: weapons run `_swImpact`, the ship's own function** (owner: *"weapons hitting
the water should be the exact same as the ship, actually"*), with a footprint their own size and an
`ampK` scale. They get the crater+rim, the velocity gash, the bent/warped/fractal sheet, and on the way
out the shed sheet.

⚠ **The amplitude then comes from the FOOTPRINT, not from the yield's `A` term** —
`baseA ∝ fp.DRAFT = R*0.19`. Tuning `A`, which is what I did first, moved almost nothing. Against the
hull's own impact seed of **0.0806**:

| | before | after (R base 30→45, ampK 0.15→0.55) |
| --- | --- | --- |
| Gatling | 0.0049 (16× weaker than the ship) | **0.0271** |
| Blaster | 0.0097 | 0.0531 |
| Thermite | 0.0142 | **0.0783** (≈ a hull impact) |
| Railgun charged | 0.0279 | 0.1533 |

Also in: a **vent** (deferred collapse column, reusing `_swWorthingtonJet`) — ⚠ my first threshold
(0.055) was above *every weapon in the game*, so it would have been dead code; 0.009 splits Gatling/
Zapper (pock) from Blaster and up (collapse). An **exit plume** for firing up from below, which produced
literally nothing before because the emitter takes its intensity from the *downward* normal and the
helper forced that sign — ⚠ and I then gated it on `_swSubmerged`, the one condition it exists for.
A **projectile wake** (`_swWpnWake`) for anything skimming the surface, matching the hull's near-water
wake. And `_swBlast` for explosions: crater+rim above, dome+vent below, deferred column.

⚠ **A budget must thin an effect, not delete it.** Both weapon budgets were early returns, so once a
250 ms window filled every further hit produced a ripple and nothing visible — in a firefight, most of
them. They scale the burst with a floor now. The pool had the headroom all along: 218–385 live against
a cap of 4000.

### Knobs added

`disp` `gain` `crestQ` `foamSlope` `steepFoam` `normK` · `rooster` `dots` · `blast` `wpn` `vent`
`wpnWake` `wxCap` `wxSeedCap` `spdTail`
...
- **(v46.76, same day) Building coverage:** the region query's core is `nearRadius + 0.5` cells wide, the near ring `nearRadius`, so one cell of drift left the ring's far edge with the wide query's towers only, and "has a `_bldList`" counted as "has its city" so it stayed that way. `_bldCoreKeys` / `__bldComplete` / `_clearBuildings()` + the re-fetch on any cell change (guarded by `_bldRegionPending`) fix it. **Water finder:** flood into `<= level + 0.5` from the seed set, then an R = 8 texel closing (`window.__earthWaterClose`); harbour uncarved-at-surface 17% → 1.2%. **Skybox leak:** `cyberpunk.jpg` stayed visible under a live weather dome after the `_wxDispose → _wxInit` cycle the water build causes; `_skyDomeRefresh` now gates on `_WX.on && _WX.dome` and the Earth tick self-heals it. **Earth ad ground:** `uAdsGFade` (0.01, 0.03) per Earth material (`window.__ads.earthGfadeOut/In`). Open: a faint far-field marbling on daylight water at altitude (exclusion list in memory).
- **(v46.77)** Earth traffic tows ad banners (`_elSpawn` → `_hcAttachBanner`, every third hull; `_elDetach` disposes them). **Ripple window rim = sponge, not wall:** `_SW_RIPPLE_FRAG`'s `nh *= edg` (a 6%-wide hard zero on the new height only) reflected every wake off the window's square; replaced by a quadratic damping of both height fields over the outer 18% (`uSpongeW/uSpongeK`, `window.__water.spongeW/spongeK`). `__waterProbe()` now reports `bandEdge/bandInner` RMS; measured rim energy 5% of interior with the sponge vs 56% with the wall.
- **(v46.77)** ripple `viscosity` 0.9945 → 0.9970 (wake half-life ~2 → ~4 s). Wake angle unchanged: Rabaud–Moisy narrowing (v41.04) gives a 20° V at the hull's on-water 350 u/s (Fr 0.97); `window.__water.waveFrc = 1.3` holds Kelvin's 39° up to cruise. The sim's wave speed is ~956 u/s, so only projectiles make a Mach cone.
- **(v46.77) The 39° wake:** the ripple sim is non-dispersive, so a hull only leaves a stationary V by outrunning its waves (a Mach cone at asin(c/v)); while a hull lays a wake the sim's wave speed now follows it, c = v·sin 19.47° (`window.__water.kelvinSin`, floor `waveCMin`, `waveC2` pins). Player stamps in the wake step, entities in `_swEntityWaterTick` on the wake condition, reader in `_swRippleTick` (1 s freshness, eased 2/s, back to 0.26 off the water). Measured: 117 u/s under a 350 u/s hull. Cost: skimming projectiles cone narrow while a hull is on the water.
- **(v46.77) Wake look:** whitecaps in `_SW_RIPPLE_FRAG` (foam from crest slope, `uBreakK/uBreakThr`, `window.__water.breakK/breakThr`) so the cone's arms whiten and the interior stays dark; `_SW_RIPPLE_RES` 512 on desktop (a `let`, set in `_swRippleInit`, `window.__water.res`); entity wake seeds every 0.12 s; entity spray default 0.6.

- **(v46.78)** **Hull-shaped water displacement.** `_swHullSilhouette` renders the player hull's underside from below (hull frame, 64x64 `MeshDepthMaterial`) and `_SW_RIPPLE_FRAG` turns it into a moving pressure patch: hollow (`-immersion*k`), rim (displaced water stood up around the body) and a closing tail (the transom section shrinking to a point `tailLen` behind = the towed hollow crashing together on the centreline); the change between poses is CARRIED (both height fields shift, `uHullKick` keeps a fraction as a pluck) - the pluck form left a filled mound behind the ship. `_swCrashSpray` = inward-flying droplets at the apex. Knobs `window.__water.hullPress/hullRim/hullTail/hullTailK/hullKick/hullFoam/hullCrest/crashSpray/hullRes/hullRedo`; debug `__hullP`, `__hullSilDump()`, `__seedLog()` (with `__seedLogOn`). **Entity-wake runaway fixed:** a wild leviathan's wake seeds (heft 8.5, no cap, no depth term) drove the field to -48 (a saturated plateau dragged behind the ship); `_swEntityWaterTick` now caps a seed at `entAmpCap` 0.08 and fades it with depth, and the sim clamps `|h| <= uHClamp` (1.2, `hClamp`) as a safety net.
- **(v46.78 cont.)** Rooster tail spawns at the closing hollow's apex when the hull patch is on (`roosterAt`); hull SKIM pull so a hovering hull still carves a shallow V (`skimDepth`/`skimH`, fades 2.5 drafts up); tail heights linear in hull speed via `_swTailUp` (`tailH`, 0.12 u of height per u/s) for both `_swRooster` and `_swCrashSpray`.
- **(v46.79)** **Stray "<ability> ACTIVE" banner on a ship change.** The banner is `Overlays.abilityFlash`, reachable only through `activateAbility`, which was gated on dead/warmup/cooldown and nothing else. Traced live (banner wrapped to log its stack): the strays were ability presses / hold-prime releases landing around the swap, and the keyboard dispatcher never checked for a menu. Now `_abilitiesBlockedByUi()` (settings, picker, lobby, `select`/`matchEnd`) and `_abilitySwapLocked()` (0.6 s of game time after `commitLoadout`) gate `activateAbility` and both `abilityInput*` entry points; the commit also drops hold primes, releases the bound ability keys and re-syncs the gamepad edge state.

- **(v46.80) EVERY WEAPON'S WATER IMPACT HAD BEEN RUNNING AT 1/20.** Owner: *"the laser type weapons aren't making ripples in the water ... blaster's weapons and abilities ... vortex main weapon, syphon main weapon, and rail gun is a tiny ripple ... i don't even think the rockets are making ripple when they hit the water (just their explosion does, and they cause wake)"*. `_swImpact` is the one "something hit the water" function and `_swWpnCross` has routed every weapon through it since v41.27 — but its `baseA` multiplied by **`window.__water.wake`**, which ships at **0.05**. `wake` is not a global water scale, it is the **hull's** calibration (the v44.82 note: the hull's wake seeds run ~20× past the display's ceiling), so recalibrating the hull silently divided every weapon by twenty and the v41.28 weapon table — measured and shipped at `wake = 1` — has been a twentieth of itself ever since. **The report's ORDERING is the proof:** the four hitscan mains rank exactly by damage (85 / 240 / 380 / 1000) and only the railgun's 0.0018 was big enough to see, while the two paths the owner says DO work — `_swBlast` (0.55, the seed clamp) and `_swWpnWake` — are precisely the two that never multiplied by `wake`. Fix is at the **caller**, not the knob: `_swImpact(..., ampK, wakeK)`, `wakeK` defaulting to `__water.wake` so every hull-family caller is bit-for-bit unchanged, and `_swWpnCross` passing 1. Measured, Gatling straight down into the hub lake with `wake` still 0.05: crater seed **−0.0008 → −0.0150** at radius 194.
- **(v46.80) Multiplayer parity, two gaps.** The peer-tracer hook passed a flat yield of **120 for every weapon** (no damage travels with `fire_tracer`), so a peer's railgun and Gatling punched identical holes — it now resolves `LOADOUTS[evt.lo].weapon` (the loadout was already in the event, read a few lines below for the railgun spiral) and takes the same `mainK` trim the local main gun does. And the peer **Vortex Laser ability had no water hook at all**: the local one has had `'vLaser'` at yield 2400 since v41.78, while the `effect_spawn / vortex_laser_beam` branch that draws the same beam for everyone else had none — the game's biggest beam reaching the lake only as its own thin tracer. ⚠ Stipple keys are **per peer** now (`'peerTracer' + fromPeerId`); a shared key meant six players firing over the lake laid one sparse trail between them, the same defect v41.37 fixed for projectiles.
- **(v46.80) `viscosity` 0.9970 → 0.9940 — the jello the fix exposed.** Owner immediately after: *"when i shoot it, it bobs up and down too much for too long ... the surface looks like jello"*. **Neither change was wrong alone.** v46.77 raised viscosity judged on a WAKE (one hull laying a smooth train, half-life 2.1 → 3.9 s); v46.80 gave weapons back their 20×, so the lake is now struck properly twenty times a second into a field that holds energy twice as long. The measurement that names it: one burst then nothing driving it — the PEAK fell 0.236 → 0.101 over 3 s while **`hRms` sat flat at ~0.015**, i.e. energy spreading rather than dissipating. `visc^60` is the per-second factor: 0.9970 → ×0.835/s (3.85 s half-life), **0.9940 → ×0.697/s (1.92 s)**, owner: *"you just fixed it"*. ⚠ One constant damps every wavelength equally, so **the ship's wake shortens with the ringing** — taken knowingly. The fix that separates them, if the wake is later judged too short, is viscous damping by **curvature** (`nh += uViscK * (lapH - lapV)`, ν∇² on the velocity term — real dissipation goes as ν·k², so chop dies fast and a long wake train does not), and it costs **no extra texture fetches**: `_SW_RIPPLE_FRAG` already reads the four neighbours as `vec4` and discards the `.y` channel the second Laplacian needs.
- **(v46.80) THE BODY-SHAPED PATCH FOR MONSTERS.** Owner: *"Monsters have a huge bowl shape around them on the surface of the water, it doesn't match their body, we fixed the ships, let's fix the monsters to be the same"* — v46.78's one open item. The bowl was literal: `_swEntityWaterTick` lays ONE round seed of radius `fp.BEAM * 1.25` (a 462-unit circle on a size-672 leviathan). `_hImm` / `_hullP` now take their silhouette, rect and y-range as **arguments** rather than reading one fixed uniform set, so one implementation serves the hull plus two entity slots (`uEntTex0/1`, `uEntA[2]`…`uEntT[2]`); `_swHullSilhouette(mesh, slot)` caches per slot; selection runs **before** the budget-gated round-robin loop (deciding mid-rotation gives a body a patch one frame and a bowl the next) and scores `BEAM / distance`. A/B on one monster, `__water.entPress` 1 vs 0: **hMin −0.157 vs −1.200** and hRms 0.0029 vs 0.0995 — and **−1.200 is exactly `uHClamp`**, so the old bowl was pinned against the v46.78 safety clamp as a flat-bottomed saturated plateau, which is what "huge bowl" meant. ⚠ A fully-submerged body **keeps** the round seed: `_hImm` saturates to the whole body height at every texel and the patch is a bowl again, just an expensive one. ⚠ The hollow is capped in **sim units** (`entHollow` 0.22, ~2× the ship's) — the 1/170 constant is calibrated on a hull's ~20-unit draft and the display's expander `h(1+11|h|)` is quadratic, so the same constant on 140 units of wading leviathan asks for h = 0.82.
- **⚠⚠⚠ (v46.80) THREE SKINNED-MESH TRAPS in `_swHullSilhouette` — every ship in this game is rigid, so none had ever fired.** (a) a leviathan has **no `userData.bboxSize` child** (that marker is written by `buildModelShipMesh`; a monster's mesh is a plain Group holding a scaled GLTF clone), so the ship-hull lookup returned null and the body silently kept its round seed; fall back to the mesh itself. (b) **a skinned body has no shape in its geometry** — cloning `o.geometry` into a plain `THREE.Mesh` draws the BIND POSE and `geometry.boundingBox` measures it: **8.9 × 5.3 × 11.2 world units for a size-780 animal** (bind pose authored near unit scale, group scale 458.8, the mesh's own world scale 4.43 — the BONES carry the rest). Clone as a `SkinnedMesh` sharing the skeleton and take the box from `SkinnedMesh.computeBoundingBox()` (r165 has it; plain `Mesh` does not), which walks vertices through their bones into the mesh's local space; ⚠ never call `bind()` without an explicit matrix, it runs `skeleton.calculateInverses()` and rewrites the LIVE animal's bind pose. (c) **a skinned mesh cannot be posed by writing its `matrixWorld`** — three recomputes `bindMatrixInverse` from that same matrix inside `SkinnedMesh.updateMatrixWorld` every render, exactly cancelling the frame transform, so the body lands at its real world position and the frame-space ortho camera is aimed at the origin 30 km away and **the RT comes back EMPTY with a perfectly correct rect and box** (the box is CPU-side and never goes near the render). Fix: draw in **world space** (`matrixWorld = identity` makes `bindMatrixInverse` identity and the shader's skinned vertex IS the world-space one; a rigid mesh gets there with `o.matrixWorld`) and put `frame` on the **camera** (`cam.matrixWorld = frame · localCam`; ⚠ clear `matrixWorldNeedsUpdate`, which `updateMatrix()` raises). **Provably a no-op for ships:** old `modelView = inv(localCam)·(inv·o.mW)`, new `= inv(frame·localCam)·o.mW = inv(localCam)·inv·o.mW`.
- **⚠⚠ (v46.80) AN EMPTY SILHOUETTE MUST FAIL BACK TO THE ROUND SEED, NOT TO NOTHING.** Trap (c) produced a well-formed record whose texture was blank, and the consequence was **worse than the bug it was fixing** — a patched body skips its round seed, so the monster stopped disturbing the water at all (owner: *"they aren't jiggling the water right now"*). Nothing threw and nothing logged; the only way to see it was to dump the texture by hand. `_swHullSilhouette` now reads the RT back once per body and counts drawn texels (`window.__water.silMinFill` 0.004, `_swRipple.silFill`); below that it returns `null`, the caller's `_ok` stays false and the body keeps the seed it has always had. Healthy readings are `fill` 0.13–0.14.
- **(v46.80) Instruments.** `window.__entP` (both entity slots: id, k, wl, tailLen, rimR, sil.fill/beam/len) · `window.__hullSilDump(cols, slot)` — slot 1/2 are the entity slots, **all dots = nothing drawn** · `window.__water.entPress` 0/1 for a live patch-vs-bowl A/B with no reload · `window.__water.visc` is read every tick, so decay is live-tunable. ⚠ **`__waterProbe()` returns all zeros when there is no water in the ripple window** (the mask early-outs every texel) — that means "you are not over water", not "the sim is broken"; I read it the wrong way once. ⚠ To ask "what is driving the surface", reset `__waterFX(true)` and run `__seedLogOn = true` / `__seedLog()` for a few seconds and bin the seeds by **distance from the player** — a 5 s census is what showed the ringing was `blast` 8 / `blastColumn` 5 / `wpnCross` 1, i.e. not weapons at all.

---

## v46.82 — RACE CIRCUITS: the Earth Circuit and the Overworld Circuit

**Jump:** `const RACE_CIRCUIT = {` (the module, right after `_raceFinishUnlocked`) · `MAP_DATA.race_earth` / `MAP_DATA.race_overworld` · `function _lssHubWorld` · `function _raceCircuitBotWaypoint` · `function _raceCircuitNetTick` · `window.__race`

Owner: *"races in custom locations... a circuit through whatever place you choose to drop into... capture the rings in order. A ring will appear when it is next to capture... then the final objective (the champion field). The positions of the rings/circuit will be identical for everyone... rings appear low and between buildings, with a ring up high every now and then. Rings will always appear on the mini map... open up the overworld in race mode, with a massive circuit through all the cities."* And, mid-build: *"bots in race mode, first priority should be the rings, sub task is to kill the enemy."*

- **Shape.** `game.raceCircuit` = one shared lap (`rings[{x,y,z,d,tx,tz,high}]`, `start`, `fin`) and every racer carries its own cursor (`_raceIdx` on `player`, each `Bot`, each `NetworkPlayer`). All the `RaceRing` objects are built at once (they share the pinned cyanring program) but `RaceRing.update` draws only ring[cursor] for the local pilot and captures for them alone; a capture advances the cursor. Past the last ring `_raceCircuitAnyCleared` opens `_raceFinishUnlocked`, the stasis owner spawns the champion field at `fin`, and `_raceCanClaim` refuses the field to anyone still on the lap (proxy bots never claim; a peer's ship claims on what its `race_prog` said).
- **Two maps.** `race_earth` (type gmaps + `stream: true` + `circuit: 'earth'`): the streamed Earth world at the DROP location (the panel relabels to RACE CIRCUIT and hides the finish box; `gmaps_user` keeps the old start→finish race). 12 gates on a wobbly 0.8–1.06 km loop; low gates 36–62 m up, high every 4th at roof+110–190 m. `race_overworld` (`Object.assign({}, hub_overworld, {circuit:'overworld'})`): 7 cities × 3 gates + 7 high transit gates = 28. **`_lssHubWorld()`** replaces `LSS.MODE === 'freeflight'` at the 8 world gates the hub needs (zone morph, hub clipmap, `nonSpawnRooms`, basin/hub clouds, gas, stream budget, `_lssFlightArena`); `_raceCircuitOverworldBuild` runs in `buildRoomGraphLevel` after `initSandwichTerrain` and builds all six sector cities through `_owEnsureSites`/`_owBuild`; `_owRaceFrame` keeps them lit and sun-shaded and nothing else (no carriers, fleets or boss - `_owFrame` stays free-flight only).
- **Placement** (`_rcPlaceRing`): a golden-angle spiral of 28 candidates around the route vertex; the nearest whose clearance from every building rising above the ring's BOTTOM exceeds ring radius + margin wins (Earth reads the patch `_cgrid` polygons in local metres; the overworld reads the city `hash` OBBs); nothing clears → climb 14 m/120 u steps; roof fallback always clears. Measured Toronto: low gates 38–58 m AGL with the tallest roof 74–220 m ABOVE them, high gates 64–99 m over the roofs.
- **⭐⭐⭐ Who decides.** The stasis owner generates (Earth: at the end of `_lssGmapsBuildLevel`, polling up to 14 s for the centre patch's buildings; overworld: synchronously in the build) and decrees `race_circuit` at 1.5 Hz until every peer acks (`race_circuit_ack`); a peer without a lap asks with `race_circuit_req` every 2 s; racers broadcast `race_prog` on each capture. The Earth curtain (`hideLoadingOverlay`) holds for the lap too (`_noCircuit`). The v43.24 rule, no pairwise arbitration.
- **⚠⚠ THREE THINGS THAT BROKE, all measured on the pane:** (1) `_lssGmapsApplyTerrainFollow` (the point-to-point race's shifter, on `_lssGmapsRefinePivot`'s 1 s timer) slid the finish room, the start grid, the pilot AND every bot 2,024 units down into the city while the gates stayed put - it now returns for circuits. (2) **THE RACE STUCK-DETECTOR COLLISION, older than this build:** the v32.71 race-waypoint detector held `game.time` in `this._stuckT` / a Vector3 in `this._stuckPos` - the SAME fields v41.93's `_unstickTick` uses as a seconds accumulator and a sample point. With a waypoint set, `_unstickTick` read `expect = speed × 125 s` on every sample, judged every race bot stuck and re-fired its escape forever (targetDir lerped to the escape at dt×4, beating the fork's dt×2): the roster orbited at full speed and never reached a gate. Renamed to `_rsWpPos`/`_rsWpT`; **this fixes Pole Position and the Shifting Run's bots too.** (3) Five cleared bots contesting a 50-unit finish dot shoved every holder out before the 0.5 s charge (CLAIM BROKEN on repeat, no round end): a circuit finish's `holdRadius` is 240 (`StasisField` ctor) and cleared bots take the v41.99 escort ring around a friendly holder.
- **Bots** (`_raceCircuitBotWaypoint`, the first branch of the race block in `Bot.update`): the next gate, approached from IN FRONT along its axis and FROM ABOVE (the approach point climbs 0.6 × distance up to 900 u - every placed gate has a clear column over it, a street-height approach point is behind a tower from most bearings; measured: a bot wedged on a hub podium at 20 u/s until this); the goal never sits below `ground under the bot + 170` (the terrain hover pad zeroed vel.y and bots crawled at 5–60 u/s down the lakeshore slope); a 760 u obstacle ray turns the goal up, then to either side, HELD 0.5 s; a stuck escape (speed < 70 for 1.2 s) goes up with a velocity kick. Bots capture at 0.62 × d (pilots 0.45). Dead racers come back after 4 s at the last gate they took, cursor intact (pilot via `respawnPlayer`, bots re-dealt in the same seat - the assault pattern); the round-end wipe rule is off for circuits. Rings first, guns opportunistic: the waypoint owns steering every frame, fire is the normal in-front-in-range path. Measured Earth lap: the roster clears in ~100 s; the overworld hub's three gates in ~35 s.
- **HUD / radar.** `#race-hud` (GATE n / N · distance · ▲ HIGH; CIRCUIT CLEAR · TAKE THE FINISH) pinned under `#round-info`; the radar draws the whole lap as a faint polyline, taken gates dim, the NEXT gate pulsing and edge-arrowed off the disc, the finish gold once clear; range 5,200 (Earth) / 14,000 (overworld). `window.__race.info()/rings()/skip()/tp(i)/regen()`.
- **Known / left as is:** the ship cruises on its own in the hub-race flight model, so an unattended pilot drifts through gates; the 3000 HP champion shell still seals the finish (existing race rule); `_raceCircuitOnLocalCapture` pushes the cursor one past N on a repeated skip (harmless, every test is `>= N`).

### v46.83 — race circuits, round two (owner's playtest notes)

**Jump:** `function _raceFinishExposed` · `function _raceRespawnPuff` · `function _raceGateLib` · `RaceRing._buildGateMesh` · `function _raceCircuitAuthority` · `window.__race.kill/killBot/expose`

- **Respawn where you died, 1.5 s, in a puff of smoke** (owner). `_raceCircuitRespawnTick` latches the death point on the first dead frame (`game._raceDeathPos`, `bot._raceDeathPos`); the pilot comes back through `respawnPlayer` behind the `game._raceRespawnInPlace` latch (a round-reset respawn keeps its spawn pick), our bots as a fresh hull in the same seat at that point; `_raceRespawnPuff` = a dozen pale cloud bursts + a light + `phase_dash`. Cursor intact. Measured: pilot back 42 u from the death point, bot seat re-dealt with its gate count.
- **The finish EXPOSED = no respawns** (owner: "if you die when the champion field is exposed, then you don't get to respawn"). `_raceFinishExposed()` = field alive and its shell down - the codebase's own word ("destroy the shell to expose the field"). The dead stay dead (NO RESPAWN banner) and the classic fleet-wipe ending returns for that phase, so a wiped fleet loses. Measured: sealed → a killed bot respawned; exposed → the bot and the pilot stayed dead, wiping fleet B ended the round 1-0.
- **Gates wear the endless Aegis bolt** (owner: "i just like the look of the powerup over the ring, and looks omni directional"). `_raceGateLib` is `_lssEndlessBoltSlot`'s exact recipe (octahedron core, fireball/plasma FX shells, glow sprite, three god-ray shafts) built once; `RaceRing({gate:true})` instances it at `diameter / 300` and animates it like `_aegis` (flicker, counter-rotating shells, precessing rays, a 12 u bob). It still acts as a ring: the capture is the sphere test. ⚠ Shared library: a gate's `destroy()` removes its group and disposes nothing. Capture cue = `rearm_reset` (the bolt's own) + its `fireball_cyan` flash.
- **⭐⭐⭐ The lap authority is the peer LONGEST IN THE ROOM**, not `amStasisOwner` (owner: "i saw someone capture my point, and it disappeared... it should be individual"). Gates are per client - a peer flying through yours cannot hide it - but the stasis-owner rule sorts random ids, so a drop-in joiner could win the lap, decree its own copy and make the host adopt it mid-race: gates cleared, cursor back to 1. `_raceCircuitAuthority` is v44.61's election with the `unknown` guard (keep what you hold); the authority never adopts; an identical lap (same kind, seed, ring count - the deterministic overworld) is re-tagged with nobody moved or reset.
- **The line to the next gate** (owner, second overworld city: "couldn't find the next ring, the line to the next one didn't appear on the map"): the radar now draws a dashed guide from the ship to the next gate, clamped at the rim when it is off the disc (the lap polyline only joined on-disc gates), the rim arrow gets a ring, and the HUD carries the bearing off the nose and a climb/dive cue: `GATE 3 / 28 · 540 M · 20° ▶ DOWN`. Measured: 40/40 sampled radar pixels cyan along the guide vs 6/40 on a control bearing; off-disc at 30 km the guide reaches the rim and the HUD reads `◀ 90°`, `▲` once faced.

### v46.84 — Pole Position and the Shifting Run are track circuits; the Shifting Run is 3x longer; the race chevron

**Jump:** `function _rcGenTrack` · `function _raceCircuitTrackBuild` · `Bot.chooseRaceWaypointTo` · `function _rcTrackNav` · the `_navMode === 'race'` branch of the radar's ROUTE-DIRECTION block

- **`circuit: 'track'`** on `race_pole_position` and `race_shifting` (owner: "should also be setup the same way, with the aegis captures instead of rings, and the other rules we just made"). The gates are the map's own rooms in TRACK ORDER - the BFS path from a start room to the champion room through `game.raceGraph` (the Shifting Run's `gate: n` numbering is that order) - a bolt at each room centre (`d = min(r × 1.5, 520)`), the champion room the finish. Laid by every client itself in `buildRoomGraphLevel` right after `_buildRaceGraph` (authored, or generated from the shared seed, so no decree: the lap is `local` and the net tick / handlers stand down for it). `_raceCircuitInstall` keeps the map's spawn rooms and champion room for a track (only the clock goes). Every other circuit rule applies unchanged: one gate per racer, the finish opens when any racer clears, `_raceCanClaim`, in-place respawns, the exposed finish is final, the 240 hold radius, the guide line and the HUD.
- **Bots keep the tunnel navigation.** `chooseRaceWaypoint()` became `chooseRaceWaypointTo(tid)` (the same sticky-room BFS aimed at any room); a track bot is sent to the room of its NEXT gate - the BFS's own "inside the target room -> its centre" step is the capture run - and, once clear, through the tunnels to the finish room where the field rules take over. Straight-line steering would ram cavern walls, which is why the track kind bypasses the from-above approach. Measured: Pole Position, 13 gates `room_1 room_4 room_5 ... room_15`, four of five bots at gates 5-6 by 35 s (one lagged at gate 2 and recovered on its own, room to room); Shifting Run, bots lap all 12 gates in ~40 s, zero stuck escapes.
