...
def main():
    ...
    # (v42.84) THE CINE FORK. Owner: "we should fork the game to make a version that can easily
    # generate really cool cinematics... it just has to be an html with a new name, in the same
    # directory."
    # It is emitted HERE, from the same stripped page, rather than kept as a hand-copied
    # index.html. A copy would be a real fork for about a week: the next water/ship/leviathan fix
    # lands in index.html only, and the trailer build quietly starts filming an older game. Built
    # this way, cine.html is a separate entry point that cannot drift - open index.html for the
    # untouched game, open cine.html for the game PLUS cine.js. The fork lives in the ENTRY POINT,
    # not in a duplicated 6 MB engine.
    # cine.js is hand-written and is NOT generated by this script: it is purely additive, and the
    # game has no knowledge that it exists.
    if js is not None:
        _tag = '<script src="lss.js?v=%s"></script>' % build
        try:
            _i = html.index(_tag)
        except ValueError:
            sys.stderr.write("cine: lss.js tag not found; skipping cine.html\n")
        else:
            _cine = html[:_i + 1] + ['<script src="cine.js?v=%s"></script>' % build] + html[_i + 1:]
            _cp = os.path.join(os.path.dirname(out), "cine.html")
            open(_cp, "wb").write(nl.join(_cine).encode("utf-8"))
            sys.stderr.write(f"wrote {_cp} ({len(_cine)} lines, + cine.js)\n")
