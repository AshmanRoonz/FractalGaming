...
- **(v35.44) The compass is part of the radar**, not a separate strip: `compass` is placed CONCENTRIC with `minimap` (both `bc`, y `-19`) and `_hlCompass` draws a rose on the disc's rim — graduations every 15°, cardinals at outer radius 13.5 against the disc's 11, plus a cyan heading marker. **⚠ The cardinals are FIXED and the marker moves.** `updateMinimap` plots with no rotation, so the disc is world-aligned with north (world −Z) permanently up; rotating the letters would disagree with the map under them. The map-name label is gone from the radar (`game.mapWord` is still maintained — that was its only reader), and `#round-info` went back to `top:10px` once the compass vacated top centre.
