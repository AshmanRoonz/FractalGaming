...
- **(v35.20) Main-menu layout.** `#btn-quick-match` + `#btn-create-room` **deleted** — the single CREATE / JOIN button `_unifyLobbyBox` builds does both (blank field hosts, typed code joins). Every JS reference (`_lobbyRenderBrowsing`, `_lobbyRenderInRoom`, the click wiring) was already null-guarded, so they no-op. `#lobby-room-box` moved out of `#lobby-grid` and into `#lobby-user-row` (the absolute top band, `top:26px`), which now spans `left:372px → right:44px` so the bar fills the middle with SETTINGS pinned at the end. The bar is a horizontal flex row; `_unifyLobbyBox` still pulls `#discord-identity` and `#room-code` into it.
