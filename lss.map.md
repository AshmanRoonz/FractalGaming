...
- **⚠ Two things to know before wiring an exported layout in.** (1) The controls have **no markup** — they are `document.createElement`'d by `_el()` inside the `#touch-controls` IIFE, and their geometry comes from a **CSS string array**, so there is no stylesheet file to point at. (2) **`_layoutSticks` px-pins `#tc-move`, `#tc-look` and both capture zones every pass**, overriding whatever CSS says for exactly those four (see the v35.04 note below). An exported layout will apply to the buttons immediately but will be ignored for the sticks and zones until `_layoutSticks` is taught to read it.
