...
> Companion map to the single-file WebGL game `index.html` (build **v35.07**).
...
- **⚠ (v35.07)** Field result: Chrome/Android window metrics (inner + visualViewport BOTH) can wedge stale for an ENTIRE fullscreen session — no final resize ever fires; exiting fullscreen was the only thing that refreshed them. So `_visBox` treats fullscreen as ground truth: when `document.fullscreenElement`, the visible box is the display itself → clamp to `screen.width/height`, or take the display box outright if the claims are unanimously transposed vs it (clamp-first keeps Android split-screen working). Diagnostic: `#tc-dbg` strip (`_tcDbgUpdate`) auto-shows 12 s whenever a source lied (branch != agree), or permanently with `?tcdbg=1` — shows `in/vv/scr/box + branch` for one-screenshot phone debugging.
