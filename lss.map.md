...
- **(v35.88) Nano Repair HUD overlay** — `_hlNanoOverlay` + `_hlNanoSpan` (defined right after `_hlArcBar`, called in `_hlDrawHUD`'s health draw): when the local perk has `regenPerSec`, a brighter-green band caps the lit health arc spanning repair accrued since last damage (render-side accumulator `_hlNanoSt`, per-frame accrual clamped to 0.5 s of regen so respawn jumps can't light the ring), and once `player.health > maxHealth` a PURPLE arc fills over the green, 0..full as the banked overheal runs 1×..`overhealMult`× hull; `_hlNanoSpan` walks `_hlArcBar`'s exact segment/gap/inset geometry but between two GLOBAL fractions — keep the two derivations in step if the arcbar shape ever changes.
