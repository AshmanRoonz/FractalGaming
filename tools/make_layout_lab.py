...
/* (v2) the panel FLOATS - it used to be pinned top-right, straight over the map box it was
   most often used to edit. Drag it by its head; position survives a reload. */
#lab-panel-head{display:flex;align-items:center;gap:6px;cursor:move;user-select:none;
  margin:-10px -10px 8px;padding:7px 10px;background:#18243a;border-bottom:1px solid #2b3a52;
  border-radius:7px 7px 0 0;font-size:12px;color:#8fd0ff;letter-spacing:1px;}
#lab-panel-head .grabdots{letter-spacing:2px;color:#4a6788;}
#lab-panel-head button{padding:2px 7px;font-size:12px;line-height:1;}
#lab-panel.collapsed{max-height:none;}
#lab-panel.collapsed > *:not(#lab-panel-head){display:none;}
#lab-panel.dragging{opacity:0.9;}
...
// ---------------------------------------------------------------- floating panel
var PLS = 'lss_layout_lab_panel_v1';
var head = document.getElementById('lab-panel-head');
function clampPanel(x, y){
  var w = panel.offsetWidth || 288, h = 40;
  return { x: Math.max(4, Math.min(x, window.innerWidth - w - 4)),
           y: Math.max(38, Math.min(y, window.innerHeight - h - 4)) };
}
function placePanel(x, y){
  var p = clampPanel(x, y);
  panel.style.left = p.x + 'px'; panel.style.top = p.y + 'px';
  panel.style.right = 'auto';
  try { localStorage.setItem(PLS, JSON.stringify({x:p.x, y:p.y, c:panel.classList.contains('collapsed')})); } catch(e){}
}
head.addEventListener('mousedown', function(e){
  ...
  function mv(ev){ placePanel(ev.clientX - off.x, ev.clientY - off.y); }
  function up(){ panel.classList.remove('dragging');
    window.removeEventListener('mousemove', mv); window.removeEventListener('mouseup', up); }
  window.addEventListener('mousemove', mv); window.addEventListener('mouseup', up);
});
document.getElementById('lab-panel-collapse').addEventListener('click', function(){
  panel.classList.toggle('collapsed');
  this.innerHTML = panel.classList.contains('collapsed') ? '+' : '&ndash;';
  var r = panel.getBoundingClientRect(); placePanel(r.left, r.top);
});
document.getElementById('lab-panel-close').addEventListener('click', function(){ pick(null); });
// keep it on screen if the window shrinks
window.addEventListener('resize', function(){
  var r = panel.getBoundingClientRect(); if (r.width) placePanel(r.left, r.top);
});
try {
  var sp = JSON.parse(localStorage.getItem(PLS) || 'null');
  if (sp){
    panel.style.left = sp.x + 'px'; panel.style.top = sp.y + 'px'; panel.style.right = 'auto';
    if (sp.c){ panel.classList.add('collapsed');
               document.getElementById('lab-panel-collapse').innerHTML = '+'; }
  }
} catch(e){}
...
<div id="lab-panel">
  <div id="lab-panel-head">
    <span class="grabdots">⋮⋮</span><b>SELECTED</b>
    <span style="flex:1;"></span>
    <button id="lab-panel-collapse" title="Collapse / expand">&ndash;</button>
    <button id="lab-panel-close" title="Deselect">&times;</button>
  </div>
